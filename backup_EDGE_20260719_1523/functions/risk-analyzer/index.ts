  ...
  try {
    ...
    if (!rag.data || rag.data.length === 0) {
      return new Response(JSON.stringify({
        agent: 'Risk_Analyzer',
        error: "Aucune source BEAC trouvée. Refus de répondre pour éviter hallucination.",
    ...
    const response = {
      ...
      focus: 'Risques opérationnels, marché, crédit',
      data_residency: "CEMAC",
      audit_trail: true,
    ...
    console.error('[Risk_Analyzer] Error:', e.message);
    return new Response(JSON.stringify({
      agent: 'Risk_Analyzer',
      error: e.message
    }), {
