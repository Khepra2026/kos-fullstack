...
serve(async (req) => {
  ...
  try {
    ...
    // FIX 1: Health check Big Four obligatoire
   ...
   .limit(3)
    ...
    // FIX 2: response_hash = hash de la réponse réelle
    ...
    await supabase.from('kos_audit_log').insert({
      ...
      org_id,
      agent_name: 'CyberSec',
      prompt_hash, // RGPD: hash du prompt
    ...
    return new Response(JSON.stringify({
      agent: "CyberSec",
      error: e?.message || 'Erreur interne du