...
// ============================================================================
// SUPREME PATCH v4.3 — KOS REGTECH AI™ + FR↔EN Auto-Translation
// ============================================================================
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ====================================================================
    // SUPREME PATH: kos_search_unified_v2 → regulatory_only=true
    // ====================================================================
    const searchStart = Date.now();
    const { data: unifiedResult, error: unifiedError } = await supabase.rpc(
      "kos_search_unified_v2",
      {
        p_query: query,
        p_top_k: 20,
        p_min_tier: null,
    ...
    if (unifiedError || !unifiedResult) {
      console.error("kos_search_unified_v2 error:", unifiedError);
      return new Response(
        JSON.stringify({
    ...
    if (generateReport) {
      ...
      answer = `${headerText}\n\n${disclaimer}\n\n=== ${lang === "en" ? "REGULATORY FINDINGS" : "CONSTATATIONS RÉGLEMENTAIRES"} ===\n\n` +
        ...
        (filteredCount > 0 ? `\n\n[FILTERED: ${filteredCount} procurement/AO_AMI — SUPREME PATCH v4.3]` : "") +
        `\n\nTotal corpus: ${result.total_sources || 344} documents | Latency: ${result.latency_ms || searchLatency}ms`;
    } else {
      if (sources.length > 0) {
        if (lang === "en") {
          answer = `KOS REGTECH AI™ — Regulatory Intelligence Search v4 (${result.total_sources || 344} documents, 204 authorities)\n\n` +
          ...
          answer = `KOS REGTECH AI™ — Recherche Réglementaire v4 (${result.total_sources || 344} documents, 204 autorités)\n\n` +
        ...
        answer += (filteredCount > 0 ? `\n[FILTERED: ${filteredCount} procurement/AO_AMI references removed]` : "") +
    ...
    try {
      await supabase.from("kos_routing_log").insert({
        function_name: "rag-universal-v4",
        provider: "unified-v2-supreme",
        status: "success",
    ...
    return new Response(
      JSON.stringify({
        ...
        cache_hit: false,
        engine: "unified-v2-supreme-v4.3",
        ...
        translated: lang === "en",
        pipeline: "RAG Universal v4.3 KOS REGTECH AI™ → kos_search_unified_v2 → mv_kos_doc_embeddings_v5 (regulatory only, FR↔EN translation)",
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : String(err);
    console.error("rag-universal-v4 error:", errorMessage);
