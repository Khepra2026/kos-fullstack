...
import { useKOSPhase3Expansion, type P3VueActive } from '@/hooks/useKOSPhase3Expansion';
import type { KOSAction } from '@/hooks/useKOSPhase1P0Immediate';
...
function Gauge({ value, size = 36, color = 'primary' }: { value: number; size?: number; color?: string }) {
  ...
  const strokeC: Record<string, string> = { primary: 'stroke-primary-500', accent: 'stroke-accent-500', emerald: 'stroke-emerald-500', amber: 'stroke-amber-500', red: 'stroke-red-500' };
  return (
...
function ActionCardCompact({ action, onStatusChange, onProgressionChange }: { action: KOSAction; onStatusChange: (id: string, statut: KOSAction['statut']) => void; onProgressionChange: (id: string, p: number) => void }) {
  const [expanded, setExpanded] = useState(false);
  const [progLocal, setProgLocal] = useState(action.progression);

  const handleStatusClick = (e: React.MouseEvent, statut: KOSAction['statut']) => {
    e.stopPropagation();
    ...
    <div className="bg-background-50 border border-background-200/60 rounded-lg cursor-pointer hover:border-background-300/80 transition-colors" onClick={() => setExpanded(!expanded)}>
      <div className="p-3">
        <div className="flex items-start gap-3">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 bg-emerald-100 text-emerald-700">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-0.5 flex-wrap">
              ...
              {action.id === 'MKT-A06' && (
                <span className="text-[9px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full border border-amber-200 font-medium">P2 Audit</span>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 text-[9px] text-foreground-500 flex-wrap">
              ...
              <span className="text-foreground-400">{action.responsable}</span>
            </div>
          </div>
          <div className="shrink-0 flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
            ...
            <Gauge value={action.progression} size={36} color="emerald" />
          </div>
        </div>
        {expanded && (
          <div className="mt-3 pt-3 border-t border-background-200/40">
            ...
            <div className="bg-emerald-50 border border-emerald-100 rounded p-2 mb-3">
              <p className="text-[9px] text-emerald-700 leading-relaxed"><strong>Pourquoi :</strong> {action.pourquoiAction}</p>
            </div>
            <div className="flex items-center gap-2 text-[9px] flex-wrap mb-3">
              ...
              <span className="bg-background-200 text-foreground-500 px-1.5 py-0.5 rounded-full"><strong>Standard :</strong> {action.standardVise}</span>
            </div>
            ...
            <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
              ...
              <input type="range" min="0" max="100" value={progLocal} onChange={handleSliderChange} onMouseUp={handleSliderCommit} onTouchEnd={handleSliderCommit} className="flex-1 h-1 accent-emerald-500 cursor-pointer" />
...
export default function KOSPhase3ExpansionPage() {
  const {
  ...
  } = useKOSPhase3Expansion();

  const sprintCounts = {
    s1: actions.filter(a => a.sprint === 1).length,
    s2: actions.filter(a => a.sprint === 2).length,
    s3: actions.filter(a => a.sprint === 3).length,
  };

  const handleLaunch = () => {
    showToast('Phase 3 activée — 8 actions stratégiques, 3 sprints, 180 jours. Objectif : domination continentale certifiée ISO d\'ici le 31 Juillet 2027.');
  };
  ...
  if (loading) {
    return (
      <KOSHubLayout hubId={213} activeTab="dashboard" tabLabel="Phase 3 Expansion">
      ...
      <KOSHubLayout hubId={213} activeTab="dashboard" tabLabel="Phase 3 Expansion">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <button onClick={fetchData} className="px-4 py-2 bg-emerald-500 text-white rounded-full text-xs cursor-pointer hover:bg-emerald-600 whitespace-nowrap">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <span className="text-xs tracking-widest uppercase bg-emerald-600 text-white px-3 py-1 rounded-full font-bold">PHASE 3 EXPANSION</span>
            ...
            <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">1 P2 Audit + 7 Stratégiques</span>
            <span className="text-xs bg-background-100 text-foreground-600 px-2 py-0.5 rounded-full">{meta.horizon}</span>
            <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-medium font-mono">{meta.budgetTotal}</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-heading font-bold text-foreground-950">
            Phase 3 — Expansion & Domination Continentale
          </h1>
          <p className="text-foreground-600 mt-2 max-w-4xl text-sm md:text-base leading-relaxed">
            <strong className="text-emerald-700">Au-delà du correctif.</strong> La Phase 3 marque le basculement : de la correction et l'optimisation (Phases 1-2) vers 
            <strong className="text-emerald-700"> l'expansion et la domination continentale.</strong> MKT-A06 (dernière action P2 de l'Audit Final) + 7 nouvelles actions stratégiques.
            <strong className="text-emerald-700"> 3 sprints</strong> — 
            <strong className="text-emerald-700"> 180 jours</strong> — 
            <strong className="text-emerald-700"> 32.5M FCFA.</strong>
          </p>
        </div>

        {/* ===== PANNEAU EXÉCUTION EN BLOC ===== */}
        <div className="bg-emerald-50/60 border border-emerald-200/60 rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center text-emerald-600"><i className="ri-rocket-2-line"></i></div>
              <div>
                <p className="text-xs font-bold text-emerald-800">Exécution en Bloc — Phase 3</p>
                <p className="text-[9px] text-emerald-600">{kpis.a_faire} à faire · {kpis.en_cours} en cours · {kpis.termine} terminées · {kpis.bloque} bloquées</p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
          ...
          <div className="bg-emerald-50 border border-emerald-200/40 rounded-lg p-3 text-center">
            <span className="text-2xl font-bold text-emerald-600">{kpis.progression_globale}%</span>
          ...
          <div className="bg-background-50 border border-background-200/60 rounded-lg p-3 text-center">
            <span className="text-lg font-bold text-emerald-700 font-mono">{meta.budgetTotal}</span>
            <p className="text-[10px] text-foreground-500">Budget</p>
          </div>
          <div className="bg-emerald-50/50 border border-emerald-200/30 rounded-lg p-3 text-center">
            <span className="text-2xl font-bold text-emerald-600">180</span>
            <p className="text-[10px] text-foreground-500">Jours</p>
          </div>
        </div>

        {/* ===== ALERTE GOUVERNANCE ===== */}
        <div className="bg-emerald-50 border border-emerald-200/50 rounded-lg p-4 mb-6 flex items-start gap-3">
          <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center shrink-0 text-emerald-600"><i className="ri-arrow-up-circle-line text-sm"></i></div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-emerald-800">Basculement Stratégique — Du Correctif à la Domination</p>
            <p className="text-xs text-emerald-700 mt-1 leading-relaxed">{meta.risquePrincipal}</p>
            <p className="text-xs text-emerald-700 mt-1"><strong>Gouvernance :</strong> {meta.gouvernance}</p>
            <p className="text-xs text-emerald-700 mt-1"><strong>Jalon Final :</strong> {meta.jalonFinal}</p>
          </div>
          <button onClick={handleLaunch} className="shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition-all cursor-pointer whitespace-nowrap">
            <i className="ri-play-circle-line text-sm"></i>Lancer Phase 3
          </button>
        </div>
          ...
          <>
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-foreground-950 mb-3 flex items-center gap-2">
                <i className="ri-building-4-line text-emerald-600"></i>Piliers Stratégiques — Phase 3
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="bg-emerald-50/50 border border-emerald-200/30 rounded-lg p-4">
                  <div className="w-10 h-10 rounded-xl bg-emerald-100 flex items-center justify-center mb-2 text-emerald-600"><i className="ri-globe-line text-lg"></i></div>
                  <p className="text-sm font-bold text-emerald-800 mb-1">Expansion Géographique</p>
                  <p className="text-[10px] text-emerald-700 leading-relaxed">Observatoire étendu 5+ pays Est/Australe · Baromètre panafricain · Force commerciale 3 zones</p>
                  <p className="text-[9px] text-emerald-600 mt-2 font-mono">{sprintCounts.s1} actions · Sprint 1</p>
                </div>
                <div className="bg-accent-50/50 border border-accent-200/30 rounded-lg p-4">
                  <div className="w-10 h-10 rounded-xl bg-accent-100 flex items-center justify-center mb-2 text-accent-600"><i className="ri-shield-check-line text-lg"></i></div>
                  <p className="text-sm font-bold text-accent-800 mb-1">Crédibilité Institutionnelle</p>
                  <p className="text-[10px] text-accent-700 leading-relaxed">Certification ISO 9001 + 27001 · 10 Partenariats stratégiques · KOS Academy certifiante</p>
                  <p className="text-[9px] text-accent-600 mt-2 font-mono">{sprintCounts.s2} actions · Sprint 2</p>
                </div>
                <div className="bg-primary-50/50 border border-primary-200/30 rounded-lg p-4">
                  <div className="w-10 h-10 rounded-xl bg-primary-100 flex items-center justify-center mb-2 text-primary-600"><i className="ri-megaphone-line text-lg"></i></div>
                  <p className="text-sm font-bold text-primary-800 mb-1">Rayonnement & Pilotage</p>
                  <p className="text-[10px] text-primary-700 leading-relaxed">KOS Regulatory Summit Africa · Dashboard Exécutif Consolidé · COMEX stratégique</p>
                  <p className="text-[9px] text-primary-600 mt-2 font-mono">{sprintCounts.s3} actions · Sprint 3</p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-amber-50/40 border border-amber-200/40 rounded-lg p-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  {kpis.sprint_actuel.actions.map(id => {
                    ...
                    return act ? (
                      <span key={id} className="text-[9px] px-2 py-1 rounded-full font-mono whitespace-nowrap bg-emerald-100 text-emerald-800">
                        {act.id} — {act.action.substring(0, 30)}...
                      </span>
                    ) : null;
                  })}
                </div>
            ...
            <h3 className="text-sm font-semibold text-foreground-950 mb-3 flex items-center gap-2">
              <i className="ri-rocket-line text-emerald-600"></i>8 Actions Stratégiques
            </h3>
            ...
            <div className="mt-8 p-5 bg-emerald-100/50 rounded-lg border border-emerald-200/40">
              <div className="flex items-center gap-2 mb-3">
                <i className="ri-lightbulb-line text-emerald-700 text-lg"></i>
                <span className="text-sm font-semibold text-emerald-900">Message Clé — Phase 3 Expansion</span>
              </div>
              <p className="text-xs text-emerald-800/80 leading-relaxed">{meta.messageCle}</p>
            </div>
          </>
        )}
        ...
        {vueActive === 'kanban' && (
          <div>
            <h3 className="text-sm font-semibold text-foreground-950 mb-4 flex items-center gap-2"><i className="ri-list-check-3"></i>Tableau Kanban — 8 Actions</h3>
        ...
        {vueActive === 'sprints' && (
          <div>
            <h3 className="text-sm font-semibold text-foreground-950 mb-4 flex items-center gap-2">
              <i className="ri-run-line"></i>3 Sprints · {meta.horizon}
            </h3>
                      ...
                      <div className="ml-14 space-y-1.5">
                        {sprintActs.map(a => (
                          <div key={a.id} className="flex items-center gap-2 text-[10px]">
                            <span className="font-mono w-16 text-emerald-600 font-medium">{a.id}</span>
        ...
        {vueActive === 'actions' && (
          <div>
            <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
              ...
              <div className="flex gap-2 flex-wrap">
                <input type="text" placeholder="Rechercher..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="text-xs rounded-full px-3 py-1.5 bg-background-50 border border-background-200 text-foreground-700 w-40 focus:outline-none focus:border-emerald-300 text-sm" />
                <select value={filtreSprint ?? 'all'} onChange={(e) => setFiltreSprint(e.target.value === 'all' ? null : parseInt(e.target.value))} className="text-xs rounded-full px-3 py-1.5 bg-background-50 border border-background-200 text-foreground-700 cursor-pointer text-sm">
                  ...
                  <option value="1">Sprint 1 — Fondations</option>
                  <option value="2">Sprint 2 — Crédibilité</option>
                  <option value="3">Sprint 3 — Rayonnement</option>
                </select>
                <select value={filtreStatut || 'all'} onChange={(e) => setFiltreStatut(e.target.value === 'all' ? null : e.target.value)} className="text-xs rounded-full px-3 py-1.5 bg-background-50 border border-background-200 text-foreground-700 cursor-pointer text-sm">
        ...
        {vueActive === 'dependances' && (
          <div>
            <h3 className="text-sm font-semibold text-foreground-950 mb-4 flex items-center gap-2"><i className="ri-git-branch-line"></i>Graphe de Dépendances — 8 Actions</h3>
            <p className="text-xs text-foreground-600 mb-6">La Phase 3 est relativement indépendante. Les dépendances sont légères — la plupart des actions peuvent démarrer en parallèle.</p>
            <div className="space-y-3">
              <div className="bg-emerald-50/40 border border-emerald-200/30 rounded-lg p-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  {actionsSansDependance.map(a => (
                    <span key={a.id} className="text-[10px] px-2 py-1 rounded-full font-mono bg-emerald-100 text-emerald-700">
                      {a.id} — {a.action.substring(0, 25)}...
                    </span>
                  ))}
                </div>
              </div>
              {actionsAvecDependance.length > 0 && (
                <div className="bg-amber-50/40 border border-amber-200/30 rounded-lg p-4">
                  <p className="text-xs font-semibold text-amber-800 mb-2 flex items-center gap-1.5">
                    <i className="ri-link"></i>Dépendances — {actionsAvecDependance.length} actions
                  </p>
                  <div className="space-y-2">
              ...
              <div className="bg-blue-50/40 border border-blue-200/30 rounded-lg p-4">
                <p className="text-xs font-semibold text-blue-800 mb-2 flex items-center gap-1.5">
                  <i className="ri-link-m"></i>Dépendances Externes (Phases 1 & 2)
                </p>
                <div className="space-y-1.5">
                  {dependancesBloquantes.map((rel, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-[10px] flex-wrap">
                      <span className="font-mono bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full">{rel.from}</span>
                      <i className="ri-arrow-right-line text-foreground-400"></i>
                      <span className="text-foreground-400">débloque</span>
                      {rel.to.map(t => (
                        <span key={t} className="font-mono bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full">{t}</span>
                      ))}
                    </div>
                  ))}
                </div>
