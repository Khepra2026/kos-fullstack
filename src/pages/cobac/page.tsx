...
import { useCOBACRegulations } from '@/hooks/useCOBACRegulations';
import { type COBACRegulation } from '@/mocks/cobacRegulations';

type DomaineFilter = 'Tous' | COBACRegulation['domaine'];
type NiveauFilter = 'Tous' | COBACRegulation['niveau'];
type TypeFilter = 'Tous' | COBACRegulation['type'];
...
const TYPE_COLORS: Record<string, string> = {
  Circulaire: '#991b1b', Règlement: '#0D7B5F', Instruction: '#92400e', Décision: '#4A5568', Avis: '#9B7B2C',
};

const DOMAINE_COLORS: Record<string, string> = {
  'Contrôle Interne': '#1A1A2E', Gouvernance: '#4A7A1E', 'LBC/FT': '#8B3A4A', Prudentiel: '#C2410C', Reporting: '#0D7B5F', Agrément: '#9B7B2C', 'Systèmes Paiement': '#B8543A',
};

export default function COBACDashboardPage() {
  const { i18n } = useTranslation();
  const isEn = i18n.language === 'en';

  const { regulations: COBACRegulations, loading, error, isLive, refetch } = useCOBACRegulations();
  ...
  const [selectedReg, setSelectedReg] = useState<COBACRegulation | null>(null);
  const [showingDetail, setShowingDetail] = useState(false);

  const filtered = useMemo(() => {
    return COBACRegulations.filter(r => {
  ...
  const stats = useMemo(() => ({
    total: COBACRegulations.length,
    rouge: COBACRegulations.filter(r => r.niveau === 'ROUGE').length,
    orange: COBACRegulations.filter(r => r.niveau === 'ORANGE').length,
    enVigueur: COBACRegulations.filter(r => r.statut === 'En vigueur' || r.statut === 'Révisé').length,
  }), []);

  const domaines: DomaineFilter[] = ['Tous', 'Contrôle Interne', 'Gouvernance', 'LBC/FT', 'Prudentiel', 'Reporting', 'Agrément', 'Systèmes Paiement'];
  const niveaux: NiveauFilter[] = ['Tous', 'ROUGE', 'ORANGE', 'JAUNE'];
  const types: TypeFilter[] = ['Tous', 'Circulaire', 'Règlement', 'Instruction', 'Décision', 'Avis'];

  return (
    <>
      <SeoHead
        title={isEn ? 'COBAC Regulatory Dashboard | CEMAC Banking Regulation | KHEPRA OS 2' : 'Dashboard Réglementaire COBAC | CEMAC | KHEPRA OS 2'}
        description={isEn ? 'Complete COBAC regulatory framework dashboard. Circulars, regulations, instructions governing CEMAC credit institutions. Prudential ratios, internal control, governance, AML/CFT.' : 'Dashboard complet du cadre réglementaire COBAC. Circulaires, règlements, instructions régissant les établissements de crédit CEMAC. Ratios prudentiels, contrôle interne, gouvernance, LBC/FT.'}
        keywords="COBAC, CEMAC banking regulation, COBAC circulars, R-2016/01, R-2018/01, R-2020/01, CEMAC prudential ratios, COBAC governance, COBAC AML, contrôle interne COBAC"
        canonicalPath="/cobac"
        ogType="website"
      ...
      <main className="min-h-screen bg-background-50">
        <Breadcrumb items={[{ label: isEn ? 'Home' : 'Accueil', path: '/' }, { label: 'COBAC', path: '/cobac' }]} />
            ...
            <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest text-emerald-400">{isEn ? 'CEMAC — Module M2' : 'CEMAC — Module M2'}</span>
                </div>
                <h1 className="text-2xl md:text-3xl font-bold font-heading">{isEn ? 'COBAC Regulatory Framework' : 'Cadre Réglementaire COBAC'}</h1>
                <p className="text-sm text-foreground-400 mt-1 max-w-xl">
                  {isEn ? 'Complete COBAC regulatory framework: circulars, regulations, instructions, and decisions governing credit institutions, microfinance, and payment systems in the CEMAC zone. Powered by KHEPRA OS 2.' : 'Cadre réglementaire COBAC complet : circulaires, règlements, instructions et décisions régissant les établissements de crédit, la microfinance et les systèmes de paiement en zone CEMAC. Propulsé par KHEPRA OS 2.'}
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="max-w-7xl mx-auto px-4 md:px-6 -mt-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { label: isEn ? 'Total Regulations' : 'Total Textes', value: stats.total, color: '#1A1A2E', icon: 'ri-scales-3-line' },
          ...
          <div className="bg-white rounded-2xl border border-background-200/70 p-4 md:p-5">
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
              <div className="relative flex-1 w-full">
                ...
                <input type="text" placeholder={isEn ? 'Search COBAC texts...' : 'Rechercher un texte COBAC...'} value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-full border border-background-200 text-sm outline-none focus:border-accent-500 transition-colors bg-background-50" />
              </div>
                    ...
                    <div className="flex items-start gap-3 md:gap-4 p-4 md:p-5 cursor-pointer" onClick={() => { setSelectedReg(reg); setShowingDetail(true); }}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-2 text-[10px] text-foreground-400">
                          ...
                          <span className={`font-semibold ${reg.statut === 'En vigueur' ? 'text-emerald-600' : reg.statut === 'Abrogé' ? 'text-red-500' : 'text-amber-500'}`}>{reg.statut}</span>
                        </div>
                      </div>
        ...
        <section className="max-w-7xl mx-auto px-4 md:px-6 pb-20">
          <div className="bg-foreground-950 rounded-2xl p-8 md:p-10 text-center text-white relative overflow-hidden">
            <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 30% 50%, rgba(234,88,12,0.08) 0%, transparent 60%), radial-gradient(ellipse at 70% 50%, rgba(34,197,94,0.06) 0%, transparent 60%)' }} />
            <div className="relative z-10">
              <h2 className="text-xl md:text-2xl font-bold font-heading mb-2">{isEn ? 'Master the COBAC regulatory framework' : 'Maîtrisez le cadre réglementaire COBAC'}</h2>
              <p className="text-sm text-foreground-400 mb-6 max-w-xl mx-auto">
                {isEn ? 'Activate the complete COBAC module of KHEPRA OS 2. Interactive regulations, impact analysis, compliance gap analysis, and dedicated CEMAC expert access.' : 'Activez le module COBAC complet de KHEPRA OS 2. Textes interactifs, analyses d\'impact, gap analysis conformité et accès expert CEMAC dédié.'}
              </p>
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <ul className="space-y-1.5">
                    {selectedReg.articles_cles.map((a, i) => <li key={i} className="text-sm text-foreground-700 flex items-start gap-2"><i className="ri-article-line text-foreground-400 mt-0.5" />{a}</li>)}
                  </ul>
                </div>
                <div>
