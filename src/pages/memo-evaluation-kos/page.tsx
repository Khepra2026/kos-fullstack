...
import {
  ...
  kosISOStatus,
  kosSecurityStatus,
  kosPerformanceStatus,
  kosBigFourActions,
  kosArchitectureSovereign,
  kosGrowthKPIs,
  kosRoadmap90j,
  kosStrengthsWeaknesses,
  kosBigFourComparison,
  kosMemoExecutiveSummary,
  kosPRAPCAStatus
} from '@/mocks/kosRegtechMaturityMemo';
...
export default function MemoEvaluationKOSPage() {
  ...
  const { score_global, certification, tables_supabase, hubs, agents_ia, edge_functions } = kosMemoExecutiveSummary.key_metrics;
      ...
      <section className="bg-foreground-900 border-b border-foreground-800">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-wrap items-center gap-2 text-[11px]">
            ...
            {kosMaturitySnapshot.score_trajectory.map((t, i) => (
              <span key={i} className="flex items-center gap-1">
                ...
                {i < kosMaturitySnapshot.score_trajectory.length - 1 && <span className="text-foreground-700 mx-1">→</span>}
              </span>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-8">
            ...
            <div className="rounded-2xl border-2 border-emerald-300 bg-gradient-to-br from-emerald-50 to-white p-6 md:p-8">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h2 className="text-xl font-bold text-foreground-950 font-playfair">{kosMemoExecutiveSummary.verdict}</h2>
                  <p className="text-xs text-foreground-500">{kosMemoExecutiveSummary.date} — {kosMemoExecutiveSummary.author}</p>
                </div>
              </div>
              <p className="text-sm text-foreground-700 leading-relaxed">{kosMemoExecutiveSummary.executive_conclusion}</p>
              <div className="mt-4 p-3 rounded-xl bg-emerald-100 border border-emerald-200">
                <p className="text-xs text-emerald-900 font-semibold">
                  ...
                  {kosMemoExecutiveSummary.next_critical_action}
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                {[
                  { label: 'Score Global', value: `${kosMemoExecutiveSummary.key_metrics.score_global}/100`, color: 'text-emerald-600' },
                  { label: 'Tables Supabase', value: kosMemoExecutiveSummary.key_metrics.tables_supabase.toLocaleString(), color: 'text-foreground-950' },
                  { label: 'Données Réelles', value: `${kosMemoExecutiveSummary.key_metrics.tables_with_data_pct}%`, color: 'text-green-600' },
                  { label: 'Edge Functions', value: `${kosMemoExecutiveSummary.key_metrics.edge_functions}/101`, color: 'text-amber-600' },
                  { label: 'Hubs KOS', value: kosMemoExecutiveSummary.key_metrics.hubs, color: 'text-foreground-950' },
                  { label: 'Agents IA', value: kosMemoExecutiveSummary.key_metrics.agents_ia, color: 'text-foreground-950' },
                  { label: 'Citations Vérifées', value: kosMemoExecutiveSummary.key_metrics.citations_verifiees, color: 'text-foreground-950' },
                  { label: 'Autorités', value: kosMemoExecutiveSummary.key_metrics.autorites, color: 'text-foreground-950' },
                  { label: 'ISO 27001', value: `${kosMemoExecutiveSummary.key_metrics.iso_27001}%`, color: 'text-emerald-600' },
                  { label: 'ISO 42001', value: `${kosMemoExecutiveSummary.key_metrics.iso_42001}%`, color: 'text-emerald-600' },
                  { label: 'Sécurité', value: `${kosMemoExecutiveSummary.key_metrics.securite_score}/100`, color: 'text-emerald-600' },
                  { label: 'Tests Totaux', value: kosMemoExecutiveSummary.key_metrics.tests_total, color: 'text-foreground-950' },
                  { label: 'Hooks Hybrides', value: `${kosMemoExecutiveSummary.key_metrics.hooks_hybrid_pct}%`, color: 'text-green-600' },
                  { label: 'PCA RTO', value: `${kosMemoExecutiveSummary.key_metrics.pca_rto_seconds}s`, color: 'text-emerald-600' },
                  { label: 'GEO Score', value: `${kosMemoExecutiveSummary.key_metrics.geo_score}/100`, color: 'text-emerald-600' }
                ].map((kpi) => (
            ...
            <div>
              ...
              <div className="space-y-2">
                {kosMemoExecutiveSummary.status_by_dimension.map((d) => (
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-background-200 bg-white p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 text-xs">
                {[
                  { label: 'Tables totales', value: kosInfraStats.supabase.tables_total, alert: false },
                  { label: 'Tables avec données', value: `${kosInfraStats.supabase.tables_with_data} (${kosInfraStats.supabase.ratio_pct}%)`, alert: false },
                  { label: 'Tables vides', value: kosInfraStats.supabase.tables_empty, alert: true },
                  { label: 'Enregistrements', value: `${kosInfraStats.supabase.records_total.toLocaleString()}+`, alert: false },
                  { label: 'Policies RLS', value: kosInfraStats.supabase.rls_policies, alert: false },
                  { label: 'RLS Coverage', value: `${kosInfraStats.supabase.rls_coverage_pct}%`, alert: false },
                  { label: 'Edge Functions', value: `${kosInfraStats.supabase.edge_functions}/${kosInfraStats.supabase.edge_functions_limit}`, alert: true },
                  { label: 'Cron Jobs', value: kosInfraStats.supabase.cron_jobs, alert: false }
                ].map((item) => (
            ...
            <div className="rounded-xl border border-background-200 bg-white p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 text-xs">
                {[
                  { label: 'Hubs KOS', value: kosInfraStats.frontend.hubs },
                  { label: 'React', value: `v${kosInfraStats.frontend.react_version}` },
                  { label: 'Build Status', value: kosInfraStats.frontend.build_status },
                  { label: 'Fichiers Mock', value: kosInfraStats.frontend.mocks_files },
                  { label: 'Hooks Total', value: kosInfraStats.frontend.hooks_total },
                  { label: 'Hooks Hybrides', value: `${kosInfraStats.frontend.hooks_hybrid} (${kosInfraStats.frontend.hooks_hybrid_pct}%)` },
                  { label: 'Hooks Mock-Only', value: kosInfraStats.frontend.hooks_mock_only },
                  { label: 'Tests Total', value: `${kosInfraStats.frontend.tests_total} (${kosInfraStats.frontend.tests_unit}U+${kosInfraStats.frontend.tests_integration}I)` }
                ].map((item) => (
            ...
            <div className="rounded-xl border border-background-200 bg-white p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 text-xs">
                {[
                  { label: 'Agents Total', value: kosInfraStats.agents.total },
                  { label: 'En Production', value: kosInfraStats.agents.in_production },
                  { label: 'Sous Supervision', value: kosInfraStats.agents.under_supervision },
                  { label: 'AI Registry', value: kosInfraStats.agents.registered_in_ai_registry },
                  { label: 'Équipes Autonomes', value: kosInfraStats.agents.teams_autonomous },
                  { label: 'Hallucination Rate', value: `${kosInfraStats.agents.hallucination_rate_pct}%` },
                  { label: 'ISO 42001', value: `${kosInfraStats.agents.iso_42001_score}%` },
                  { label: 'EU AI Act Art.14', value: kosInfraStats.agents.eu_ai_act_compliant ? '✓ Conforme' : '✗ Non conforme' }
                ].map((item) => (
                  <div key={item.label} className="rounded-lg bg-background-50 border border-background-100 p-3">
                    <span className="text-foreground-500 block mb-0.5 text-[10px] uppercase">{item.label}</span>
                    <span className="font-bold text-sm text-foreground-950">{item.value}</span>
            ...
            <div className="rounded-xl border-2 border-amber-200 bg-amber-50 p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs mb-4">
                {[
                  { label: 'Docker Conteneurs', value: kosInfraStats.infrastructure_sovereign.docker_containers_ready, note: 'Prêts' },
                  { label: 'Qdrant Collections', value: kosInfraStats.infrastructure_sovereign.qdrant_collections_ready, note: 'Configurées' },
                  { label: 'n8n Workflows', value: kosInfraStats.infrastructure_sovereign.n8n_workflows_ready, note: 'Prêts' },
                  { label: 'Systemd Services', value: kosInfraStats.infrastructure_sovereign.systemd_services, note: 'Configurés' }
                ].map((item) => (
        ...
        {activeTab === 'regulatory' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                { label: 'Citations Vérifiées', value: kosRegulatoryBase.citations_verified, color: 'text-emerald-600' },
                { label: 'Autorités', value: kosRegulatoryBase.authorities_covered, color: 'text-foreground-950' },
                { label: 'Textes Réglementaires', value: kosRegulatoryBase.regulations, color: 'text-foreground-950' },
                { label: 'Sanctions Documentées', value: kosRegulatoryBase.sanctions, color: 'text-foreground-950' }
              ].map((s) => (
            ...
            <div className="rounded-xl border border-background-200 bg-white overflow-hidden">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {kosRegulatoryBase.texts_verified.map((t, i) => (
            ...
            <div className="grid lg:grid-cols-3 gap-4">
              ...
              <div className="rounded-xl border-2 border-emerald-300 bg-emerald-50 p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-2xl font-black text-emerald-600">{kosISOStatus.iso_27001.score}</span>
                </div>
                <span className="px-2 py-1 rounded text-[11px] font-bold bg-emerald-200 text-emerald-800 block text-center mb-3">{kosISOStatus.iso_27001.status}</span>
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between"><span className="text-emerald-700">Contrôles</span><span className="font-bold">{kosISOStatus.iso_27001.controls_passed}/{kosISOStatus.iso_27001.controls_total}</span></div>
                  ...
                  <div className="flex justify-between"><span className="text-emerald-700">Certification cible</span><span className="font-bold">{kosISOStatus.iso_27001.certification_target}</span></div>
                  ...
                  <div className="flex justify-between"><span className="text-emerald-700">Formation</span><span className="font-bold text-amber-600">{kosISOStatus.iso_27001.evidence.staff_training_pct}% formé</span></div>
                </div>
                <div className="mt-3 p-2 rounded bg-emerald-100 border border-emerald-200 text-[10px] text-emerald-800 font-semibold">
                  Organismes recommandés : {kosISOStatus.iso_27001.recommended_bodies.join(' · ')}
                </div>
              </div>
              ...
              <div className="rounded-xl border-2 border-emerald-200 bg-white p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-2xl font-black text-emerald-600">{kosISOStatus.iso_42001.score}</span>
                </div>
                <span className="px-2 py-1 rounded text-[11px] font-bold bg-emerald-200 text-emerald-800 block text-center mb-3">{kosISOStatus.iso_42001.status}</span>
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between"><span className="text-foreground-600">Digital Twin</span><span className="font-bold text-emerald-600">{kosISOStatus.iso_42001.digital_twin_score}/10 ✓</span></div>
                  ...
                  <div className="flex justify-between"><span className="text-foreground-600">Taux hallucination</span><span className="font-bold text-emerald-600">{kosISOStatus.iso_42001.hallucination_rate_pct}% ✓</span></div>
                  <div className="flex justify-between"><span className="text-foreground-600">Agents enregistrés</span><span className="font-bold">{kosISOStatus.iso_42001.agents_registered}</span></div>
              ...
              <div className="rounded-xl border-2 border-emerald-200 bg-white p-5">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-2xl font-black text-emerald-600">{kosISOStatus.iso_9001.score}</span>
                </div>
                <span className="px-2 py-1 rounded text-[11px] font-bold bg-emerald-200 text-emerald-800 block text-center mb-3">{kosISOStatus.iso_9001.status}</span>
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between"><span className="text-foreground-600">Processus documentés</span><span className="font-bold text-emerald-600">{kosISOStatus.iso_9001.processes_documented}/26 ✓</span></div>
            ...
            <div className="rounded-xl border border-background-200 bg-white p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center text-xs">
                <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-4">
                  ...
                  <span className="text-2xl font-black text-emerald-600">{kosPRAPCAStatus.rto_seconds}s</span>
                  <span className="text-[10px] text-emerald-600 block">Cible &lt; {kosPRAPCAStatus.rto_target_seconds}s ✓</span>
                </div>
                <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-4">
                  <span className="text-[10px] uppercase font-bold text-foreground-500 block mb-1">RPO Mesuré</span>
                  <span className="text-2xl font-black text-emerald-600">{kosPRAPCAStatus.rpo_minutes}min</span>
                  <span className="text-[10px] text-emerald-600 block">Cible &lt; {kosPRAPCAStatus.rpo_target_minutes}min ✓</span>
                </div>
                <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-4">
                  <span className="text-[10px] uppercase font-bold text-foreground-500 block mb-1">Dernier Test</span>
                  <span className="text-sm font-black text-emerald-600">{kosPRAPCAStatus.last_test}</span>
                </div>
                <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-4">
                  <span className="text-[10px] uppercase font-bold text-foreground-500 block mb-1">Statut</span>
                  <span className="text-sm font-black text-emerald-600">{kosPRAPCAStatus.status}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'security' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <span className="text-3xl font-black text-emerald-600">{kosSecurityStatus.score_global}/100</span>
            </div>

            {/* Scores Grid */}
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {[
                { label: 'Global', score: kosSecurityStatus.score_global },
                { label: 'Headers HTTP', score: kosSecurityStatus.headers.score },
                { label: 'CSP Niveau 3', score: kosSecurityStatus.csp.score },
                { label: 'CORS', score: kosSecurityStatus.cors.score },
                { label: 'Cookies', score: kosSecurityStatus.cookies.score },
                { label: 'HSTS', score: kosSecurityStatus.hsts.score }
              ].map((s) => (
            ...
            <div className="rounded-xl border border-background-200 bg-white p-6">
              ...
              <div className="space-y-3 text-xs">
                {[
                  { name: 'HSTS', config: kosSecurityStatus.hsts.config, status: kosSecurityStatus.hsts.status },
                  { name: 'CSP', config: kosSecurityStatus.csp.config, status: kosSecurityStatus.csp.status },
                  { name: 'Cookies', config: 'Secure; SameSite=Strict', status: kosSecurityStatus.cookies.status },
                  { name: 'Headers HTTP', config: kosSecurityStatus.headers.status, status: 'Déployé' },
                  { name: 'WAF', config: `${kosSecurityStatus.waf.function} — ${kosSecurityStatus.waf.rules}`, status: 'Actif' },
                  { name: 'RLS', config: `${kosSecurityStatus.rls.tables} tables, ${kosSecurityStatus.rls.policies} policies`, status: `${kosSecurityStatus.rls.coverage_pct}%` }
                ].map((h) => (
            ...
            <div className="rounded-xl border border-background-200 bg-white p-5">
              ...
              <div className="flex items-start gap-2 text-xs p-3 rounded-lg bg-amber-50 border border-amber-200">
                ...
                <span className="text-amber-800">{kosSecurityStatus.penetration_test}</span>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-sm font-bold text-emerald-700 mb-3 flex items-center gap-2">
                <i className="ri-check-double-fill text-emerald-500" /> Blocs Complétés — {kosBigFourActions.completed.length} actions
              </h3>
              <div className="space-y-2">
                {kosBigFourActions.completed.map((a) => (
            ...
            <div>
              <h3 className="text-sm font-bold text-amber-700 mb-3 flex items-center gap-2">
                <i className="ri-time-line text-amber-500" /> Actions P0/P1/P2 Restantes — {kosBigFourActions.pending_p0.length} actions
              </h3>
              <div className="space-y-2">
                {kosBigFourActions.pending_p0.map((a) => (
        ...
        {activeTab === 'architecture' && (
          <div className="space-y-6">
            ...
            <div className="rounded-xl border-2 border-background-300 bg-white p-5">
              ...
              <p className="text-xs text-foreground-600 font-mono bg-background-50 p-3 rounded-lg">{kosArchitectureSovereign.target_architecture}</p>
            </div>

            <div className="space-y-3">
              {kosArchitectureSovereign.layers.map((layer) => (
        ...
        {activeTab === 'growth' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {[
                { label: 'Pipeline Total', value: `${(kosGrowthKPIs.pipeline_total_fcfa / 1e9).toFixed(2)} Md FCFA`, highlight: true },
                { label: 'Pipeline Pondéré', value: `${(kosGrowthKPIs.pipeline_pondere_fcfa / 1e9).toFixed(2)} Md FCFA`, highlight: false },
                { label: 'Deals Actifs', value: kosGrowthKPIs.deals_actifs, highlight: false },
                { label: 'Win Rate YTD', value: `${kosGrowthKPIs.win_rate_ytd_pct}%`, highlight: true },
                { label: 'Leads Actifs', value: kosGrowthKPIs.leads_actifs, highlight: false },
                { label: 'Bailleurs Trackés', value: kosGrowthKPIs.tenderers_tracked, highlight: false },
                { label: 'Bailleurs Accrédités', value: kosGrowthKPIs.donors_accredited, highlight: false },
                { label: 'Orgs Institutionnelles', value: `${(kosGrowthKPIs.institutional_orgs_tracked / 1000).toFixed(1)}K`, highlight: false },
                { label: 'Décideurs Mappés', value: `${(kosGrowthKPIs.decision_makers_mapped / 1000).toFixed(0)}K`, highlight: false },
                { label: 'Backlinks Actifs', value: kosGrowthKPIs.backlinks_active, highlight: false },
                { label: 'Domain Authority', value: kosGrowthKPIs.domain_authority, highlight: false },
                { label: 'Trafic Organique', value: `${(kosGrowthKPIs.organic_traffic_monthly / 1000).toFixed(0)}K/mois`, highlight: true },
                { label: 'Keywords Top 10', value: kosGrowthKPIs.seo_top10_keywords.toLocaleString(), highlight: false },
                { label: 'GEO Score', value: `${kosGrowthKPIs.geo_score}/100`, highlight: true },
                { label: 'Visibilité ChatGPT', value: `${kosGrowthKPIs.chatgpt_visibility_pct}%`, highlight: false },
                { label: 'Inscrits Nurturing', value: kosGrowthKPIs.nurturing_subscribers.toLocaleString(), highlight: false }
              ].map((kpi) => (
            ...
            {/* Performance */}
            <div className="rounded-xl border border-background-200 bg-white p-6">
              <h3 className="text-sm font-bold text-foreground-950 mb-4">Core Web Vitals — Dernier Scan {kosPerformanceStatus.last_scan}</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs text-center">
                <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-3">
                  <span className="text-[10px] uppercase font-bold text-foreground-500 block mb-1">Score Mobile</span>
                  <span className="text-2xl font-black text-emerald-600">{kosPerformanceStatus.homepage_mobile.score}</span>
                </div>
                <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-3">
                  <span className="text-[10px] uppercase font-bold text-foreground-500 block mb-1">LCP Mobile</span>
                  <span className="text-2xl font-black text-emerald-600">{kosPerformanceStatus.homepage_mobile.lcp_ms}ms</span>
                </div>
                <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-3">
                  <span className="text-[10px] uppercase font-bold text-foreground-500 block mb-1">TBT Mobile</span>
                  <span className="text-2xl font-black text-emerald-600">{kosPerformanceStatus.homepage_mobile.tbt_ms}ms</span>
                </div>
                <div className="rounded-lg bg-emerald-50 border border-emerald-200 p-3">
                  <span className="text-[10px] uppercase font-bold text-foreground-500 block mb-1">CWV Grade</span>
                  <span className="text-lg font-black text-emerald-600">{kosPerformanceStatus.core_web_vitals_grade}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'comparison' && (
          <div className="space-y-6">
            ...
            <div className="rounded-xl border-2 border-emerald-300 bg-emerald-50 p-4 mb-4">
              <p className="text-sm text-emerald-900 font-semibold">{kosBigFourComparison.verdict}</p>
            </div>
            ...
            <div className="rounded-xl border border-background-200 bg-white overflow-hidden">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {kosBigFourComparison.dimensions.map((d, i) => (
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-2">
                  {kosStrengthsWeaknesses.strengths.map((s) => (
              ...
              <div>
                ...
                <div className="space-y-2">
                  {kosStrengthsWeaknesses.weaknesses.map((w) => (
        ...
        {activeTab === 'roadmap' && (
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-background-200 bg-white p-5">
              ...
              <div className="space-y-2">
                {kosMaturitySnapshot.score_trajectory.map((t) => (
