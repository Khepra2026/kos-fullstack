...
import { useThinkTankAutomates } from '@/hooks/useThinkTankAutomates';
import type { KOSThinkTankAutomate } from '@/hooks/useThinkTankAutomates';
...
export default function KOSThinkTankAutomatesPage() {
  const { i18n } = useTranslation();
  const lang = i18n.language.startsWith('en') ? 'en' : 'fr';
  const { automates, categories, kpis, isLive, loading, error, refetch } = useThinkTankAutomates();
  ...
  const coreCategories = ['research-production', 'strategic-watch', 'policy-analysis'];

  return (
    <KOSHubLayout hubId={54}>
      <SeoHead
        title="KOS Think Tank Automates™ — Intelligence Collective | KHEPRA EXPERTS"
        description="Registre des 24 automates Think Tank KOS : recherche, veille stratégique, policy analysis, data intelligence, publications, événements, partenariats académiques, impact tracking. 56,420+ tâches."
        keywords="KOS Think Tank Automates, recherche automatisée, veille stratégique, policy analysis, data intelligence, publications académiques, think tank Afrique, KHEPRA EXPERTS"
        canonicalPath="/kos-think-tank-automates"
        ogType="website"
      ...
      <section className="relative pt-32 pb-16 sm:pt-40 sm:pb-20 overflow-hidden bg-foreground-950">
        <div className="absolute inset-0">
          <img
            src="https://readdy.ai/api/search-image?query=Abstract%20warm%20intellectual%20atmosphere%20with%20golden%20amber%20and%20deep%20olive%20tones%2C%20elegant%20research%20library%20visualization%20with%20glowing%20knowledge%20graphs%20and%20interconnected%20neural%20nodes%2C%20sophisticated%20academic%20aesthetic%20with%20geometric%20patterns%20suggesting%20intellectual%20capital%20and%20thought%20leadership%2C%20premium%20consulting%20think%20tank%20ambiance%20with%20rich%20textures%20and%20subtle%20layered%20depth%2C%20no%20text%20no%20human%20figures%2C%20clean%20professional%20composition&width=1920&height=600&seq=kos-thinktank-hero&orientation=landscape"
            alt=""
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
              ...
              <span className="block text-amber-400 mt-2">24 automates Think Tank.</span>
            </h1>
            <p className="text-lg sm:text-xl text-gray-300 leading-relaxed mb-8 max-w-3xl mx-auto">
              <strong className="text-white">{kpis.total_agents} automates</strong> répartis sur <strong className="text-white">{kpis.categories} domaines</strong> de recherche et prospective.{' '}
              De la veille réglementaire à la production de position papers, des partenariats académiques au tracking d'impact.{' '}
      ...
      <section className="py-4 bg-foreground-950 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-10 gap-3">
            {[
              { label: 'Total', value: String(kpis.total_agents), icon: 'ri-lightbulb-line', color: '#E8C547' },
              ...
              { label: 'Publications', value: (kpis.publications_produced / 1000).toFixed(0) + 'K', icon: 'ri-book-open-line', color: '#86BC25' },
              { label: 'Citations', value: (kpis.citations_tracked / 1000).toFixed(1) + 'K', icon: 'ri-double-quotes-r', color: '#C05A3A' },
              { label: 'Chercheurs', value: (kpis.researchers_network / 1000).toFixed(1) + 'K', icon: 'ri-team-line', color: '#059669' },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-foreground-950 mb-2">
              8 Domaines — Chaîne de Valeur Intellectuelle
            </h2>
            <p className="text-foreground-600">De la veille stratégique à la mesure d'impact, chaque maillon de la production intellectuelle est automatisé.</p>
          </div>
                ...
                >
                  {isCore && (
                    <div className="absolute top-3 right-3">
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-700 font-bold border border-amber-200">CŒUR</span>
                    </div>
                  )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-sm text-foreground-500">Chargement des 24 automates Think Tank...</p>
            </div>
          )}
            ...
            <>
              ...
              {filteredAutomates.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {filteredAutomates.map((automate: KOSThinkTankAutomate) => {
      ...
      <section className="py-12 sm:py-16 bg-background-50 border-t border-background-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-foreground-950 mb-2">
              Chaîne de Valeur Intellectuelle — 8 Maillons Automatisés
            </h2>
            <p className="text-foreground-600">De la détection des signaux faibles à la mesure d'impact sur les politiques publiques, chaque étape est orchestrée par les automates KOS.</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 max-w-6xl mx-auto">
            {[
              { id: 'strategic-watch', label: 'Veille & Prospective', icon: 'ri-radar-line', color: '#EA580C', desc: 'Réglementaire · Géopolitique · Megatrends' },
              { id: 'research-production', label: 'Recherche & Production', icon: 'ri-lightbulb-line', color: '#86BC25', desc: 'Lit. Review · Position Paper · Études' },
              { id: 'policy-analysis', label: 'Policy & Plaidoyer', icon: 'ri-scales-3-line', color: '#9B7B2C', desc: 'Impact Analysis · Drafting · Advocacy' },
              { id: 'data-intelligence', label: 'Data Intelligence', icon: 'ri-bar-chart-box-line', color: '#0D7B5F', desc: 'Économétrie · Sondages · NLP Quali' },
              { id: 'publications-diffusion', label: 'Publications & Diffusion', icon: 'ri-book-open-line', color: '#BE123C', desc: 'Mise en page · Peer Review · Distribution' },
              { id: 'events-conferences', label: 'Événements', icon: 'ri-calendar-event-line', color: '#8B3040', desc: 'Planification · Contenu · Hybride' },
              { id: 'academic-partnerships', label: 'Partenariats', icon: 'ri-team-line', color: '#059669', desc: 'Réseau · Co-publications · Doctoral' },
              { id: 'impact-citation', label: 'Impact & Citations', icon: 'ri-line-chart-line', color: '#C05A3A', desc: 'Citations · Médias · Réformes' },
            ].map(layer => {
      ...
      <section className="py-12 sm:py-16 bg-white border-t border-background-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-foreground-950 mb-2">
              Écosystème KOS — Intelligence & Recherche
            </h2>
            <p className="text-foreground-600">Accès rapide aux modules connexes de recherche et d'intelligence.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Think Tank Automates', path: '/kos-think-tank-automates', icon: 'ri-lightbulb-line', color: '#E8C547', current: true },
              { label: 'Research Institute', path: '/kos-research-institute', icon: 'ri-flask-line', color: '#86BC25' },
              { label: 'Knowledge Center', path: '/kos-knowledge-center', icon: 'ri-book-2-line', color: '#059669' },
              { label: 'Market Intelligence', path: '/kos-market-intelligence-command', icon: 'ri-bar-chart-box-line', color: '#EA580C' },
              { label: 'Strategic Intelligence', path: '/kos-strategic-intelligence', icon: 'ri-brain-line', color: '#9B7B2C' },
              { label: 'AI Governance & Ethics', path: '/kos-ai-governance-ethics', icon: 'ri-scales-line', color: '#8B3040' },
              { label: 'Enterprise Brain OS', path: '/kos-enterprise-brain-os', icon: 'ri-cpu-line', color: '#C05A3A' },
              { label: 'Think Tank Public', path: '/think-tank', icon: 'ri-globe-line', color: '#BE123C' },
            ].map(link => (
