...
import { useRegulatoryComplianceAutomates } from '@/hooks/useRegulatoryComplianceAutomates';
import type { KOSRegulatoryComplianceAutomate } from '@/hooks/useRegulatoryComplianceAutomates';
...
export default function KOSRegulatoryComplianceAutomatesPage() {
  const { i18n } = useTranslation();
  const lang = i18n.language.startsWith('en') ? 'en' : 'fr';
  const { automates, categories, kpis, isLive, loading, error, refetch } = useRegulatoryComplianceAutomates();
  ...
  const coreCategories = ['veille-reglementaire', 'impact-analysis', 'compliance-documentaire', 'reporting-reglementaire'];

  return (
    <KOSHubLayout hubId={55}>
      <SeoHead
        title="KOS Regulatory Compliance Automates™ — Veille & Conformité | KHEPRA EXPERTS"
        description="Registre des 24 automates Veille Réglementaire & Conformité KOS : BCEAO, COBAC, OHADA, GAFI, LCB-FT, audit, reporting, risques, formation. 98,420+ tâches, 14 juridictions."
        keywords="KOS Regulatory Compliance Automates, veille réglementaire BCEAO COBAC, conformité OHADA GAFI, audit réglementaire automatisé, LCB-FT reporting, KHEPRA EXPERTS"
        canonicalPath="/kos-regulatory-compliance-automates"
        ogType="website"
      ...
      <section className="relative pt-32 pb-16 sm:pt-40 sm:pb-20 overflow-hidden bg-foreground-950">
        <div className="absolute inset-0">
          <img
            src="https://readdy.ai/api/search-image?query=Abstract%20regulatory%20compliance%20atmosphere%20with%20deep%20emerald%20and%20warm%20amber%20tones%2C%20elegant%20legal%20and%20governance%20visualization%20with%20interconnected%20compliance%20nodes%20and%20regulatory%20framework%20patterns%2C%20sophisticated%20audit%20and%20compliance%20aesthetic%20with%20rich%20textures%2C%20geometric%20shield%20and%20scale%20motifs%20suggesting%20protection%20and%20justice%2C%20no%20text%20no%20human%20figures%2C%20premium%20governance%20ambiance&width=1920&height=600&seq=kos-regcomp-hero&orientation=landscape"
            alt=""
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            <div className="flex flex-wrap justify-center gap-3 mb-6">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/20 border border-emerald-400/30 backdrop-blur-sm">
                ...
                <span className="text-sm font-semibold text-emerald-300 uppercase tracking-wider">
                  KOS Regulatory Compliance Automates™
                </span>
              </div>
            ...
            <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
              ...
              <span className="block text-emerald-400 mt-2">24 automates réglementaires.</span>
            </h1>
            <p className="text-lg sm:text-xl text-gray-300 leading-relaxed mb-8 max-w-3xl mx-auto">
              <strong className="text-white">{kpis.total_agents} automates</strong> répartis sur <strong className="text-white">{kpis.categories} domaines</strong> de veille réglementaire et conformité.{' '}
              Couvrant <strong className="text-white">{kpis.jurisdictions_covered} juridictions</strong> (BCEAO, COBAC, OHADA, GAFI, CEMAC...).{' '}
      ...
      <section className="py-4 bg-foreground-950 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-10 gap-3">
            {[
              { label: 'Total', value: String(kpis.total_agents), icon: 'ri-shield-check-line', color: '#059669' },
              ...
              { label: 'Obligations', value: (kpis.obligations_tracked / 1000).toFixed(1) + 'K', icon: 'ri-list-check', color: '#059669' },
              { label: 'Reports', value: (kpis.reports_generated / 1000).toFixed(1) + 'K', icon: 'ri-file-chart-line', color: '#86BC25' },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-foreground-950 mb-2">
              8 Domaines — Pyramide de Conformité Réglementaire
            </h2>
            <p className="text-foreground-600">De la veille réglementaire à la gestion des contentieux, chaque couche de la conformité est automatisée et orchestrée.</p>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-sm text-foreground-500">Chargement des 24 automates Veille Réglementaire & Conformité...</p>
            </div>
          )}
            ...
            <>
              ...
              {filteredAutomates.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {filteredAutomates.map((automate: KOSRegulatoryComplianceAutomate) => {
      ...
      <section className="py-12 sm:py-16 bg-background-50 border-t border-background-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-foreground-950 mb-2">
              Pyramide de Conformité — 8 Couches de Protection
            </h2>
            <p className="text-foreground-600">De la veille à la gestion des contentieux, une architecture de conformité multicouche orchestrée par les automates KOS.</p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 max-w-6xl mx-auto">
            {[
              { id: 'veille-reglementaire', label: 'Veille Réglementaire', icon: 'ri-radar-line', color: '#EA580C', desc: 'BCEAO · COBAC · OHADA · GAFI' },
              { id: 'impact-analysis', label: 'Analyse d\'Impact', icon: 'ri-scales-3-line', color: '#9B7B2C', desc: 'Gap · Scoring · Roadmap' },
              { id: 'compliance-documentaire', label: 'Conformité Documentaire', icon: 'ri-file-shield-2-line', color: '#059669', desc: 'Politiques · Preuves · Registres' },
              { id: 'reporting-reglementaire', label: 'Reporting Réglementaire', icon: 'ri-file-chart-line', color: '#86BC25', desc: 'SURFI · LCB-FT · ESG' },
              { id: 'audit-inspection', label: 'Audit & Inspection', icon: 'ri-search-eye-line', color: '#BE123C', desc: 'Simulation · Continu · Rapports' },
              { id: 'risque-conformite', label: 'Risques Conformité', icon: 'ri-alert-line', color: '#DC2626', desc: 'Carto · Contrôles · Incidents' },
              { id: 'formation-conformite', label: 'Formation Conformité', icon: 'ri-graduation-cap-line', color: '#0D7B5F', desc: 'Modules · Certification · Culture' },
              { id: 'juridique-contentieux', label: 'Architecture Juridique', icon: 'ri-scales-line', color: '#8B3040', desc: 'Jurisprudence · Contrats · Alertes' },
            ].map(layer => {
      ...
      <section className="py-12 sm:py-16 bg-white border-t border-background-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-foreground-950 mb-2">
              Écosystème KOS — Conformité & Régulation
            </h2>
            <p className="text-foreground-600">Accès rapide aux modules connexes de conformité et veille réglementaire.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Regulatory Compliance Automates', path: '/kos-regulatory-compliance-automates', icon: 'ri-shield-check-line', color: '#059669', current: true },
              { label: 'Regulatory Intelligence', path: '/regulatory-intelligence', icon: 'ri-auction-line', color: '#EA580C' },
              { label: 'Compliance Management', path: '/compliance-management', icon: 'ri-file-shield-2-line', color: '#86BC25' },
              { label: 'Risk & Diligence Command', path: '/kos-risk-diligence-command', icon: 'ri-alert-line', color: '#DC2626' },
              ...
              { label: 'Transfer Pricing', path: '/transfer-pricing', icon: 'ri-exchange-dollar-line', color: '#C05A3A' },
