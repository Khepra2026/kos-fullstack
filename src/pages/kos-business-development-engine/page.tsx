...
export default function KOSBusinessDevelopmentPage() {
  ...
  if (loading) return <KOSHubLayout hubId={77} activeTab="pipeline" tabLabel="Business Development"><div className="max-w-7xl mx-auto px-4 md:px-6 py-8 flex items-center justify-center py-20"><div className="flex items-center gap-3 text-foreground-500"><div className="w-5 h-5 rounded-full border-2 border-primary-500 border-t-transparent animate-spin"></div><span className="text-sm">Chargement Business Development...</span></div></div></KOSHubLayout>;
  if (error && stages.length === 0) return <KOSHubLayout hubId={77} activeTab="pipeline" tabLabel="Business Development"><div className="max-w-7xl mx-auto px-4 md:px-6 py-8 flex flex-col items-center justify-center py-20 gap-4"><div className="w-12 h-12 rounded-full bg-red-100 flex items-center justify-center text-red-600"><i className="ri-error-warning-line text-xl"></i></div><button onClick={refetch} className="px-4 py-2 rounded-full bg-primary-500 text-background-50 text-xs font-medium hover:bg-primary-600 cursor-pointer whitespace-nowrap"><i className="ri-refresh-line mr-1.5"></i>Réessayer</button></div></KOSHubLayout>;

  return (
    <KOSHubLayout hubId={77} activeTab={activeTab} tabLabel="Business Development">
