...
import { useKOSZeroBudgetSprint2 } from '@/hooks/useKOSZeroBudgetSprint2';
import { AnimatedCounter } from '@/components/base/AnimatedCounter';
import { ZeroBudgetAction2 } from '@/mocks/kosZeroBudgetSprint2';
...
export default function KOSZeroBudgetSprint2Page() {
  const {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-semibold text-foreground-950">
                  Sprint 2 Zéro Budget — Les 10 Tâches Restantes
                </h1>
                <p className="text-sm text-foreground-600">
                  {meta.mandate} · Suite du {meta.parentSprint}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="bg-accent-50 rounded-xl px-4 py-3 text-center">
                ...
                <div className="text-xs text-foreground-600">Budget Évité (Sprint 2)</div>
              </div>
              <div className="bg-primary-50 rounded-xl px-4 py-3 text-center">
                <div className="text-xl font-bold text-primary-500">
                  {meta.combinedBudgetAvoided}
                </div>
                <div className="text-xs text-foreground-600">Budget Évité (Sprints 1+2)</div>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-6 py-8">
        ...
        <div className="bg-background-50 rounded-2xl border border-background-200/70 p-6 mb-8">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-foreground-950">Progression Globale Sprint 2</h3>
          ...
          <div className="h-3 bg-background-200 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.round((kpis.executed / kpis.totalActions) * 100 + (kpis.inProgress / kpis.totalActions) * 55)}%` }}
            ></div>
          </div>
          <div className="flex justify-between mt-3 text-xs text-foreground-600">
            <span>Début: 23 Juin 08:30</span>
            <span className="text-secondary-500 font-medium">{kpis.actionsCreativeWorkaround} contournement créatif</span>
            <span>Cible: 25 Juin 18:00</span>
          </div>
        </div>

        {/* Combined Sprint Summary */}
        <div className="bg-gradient-to-r from-accent-50 to-secondary-50 rounded-2xl border border-accent-200/30 p-5 mb-8">
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-xl bg-accent-500 flex items-center justify-center shrink-0">
              <i className="ri-trophy-line text-background-50 text-lg"></i>
            </div>
            <div>
              <div className="text-sm font-semibold text-foreground-950 mb-1">Bilan Combiné Sprints 1+2</div>
              <div className="text-sm text-foreground-700">
                <strong>20 actions</strong> traitées en une journée · <strong>{meta.combinedBudgetAvoided}</strong> de budget évité · <strong>{meta.combinedValueCreated}</strong> de valeur créée — le tout à <strong>0 FCFA</strong> de dépense externe.
              </div>
            </div>
          </div>
        </div>
              ...
              >
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  ...
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
                    style={{ backgroundColor: action.category === 'security' ? '#fef2f2' : action.category === 'compliance' ? '#fefce8' : action.category === 'growth' ? '#f0fdf4' : action.category === 'code' ? '#f1f5f9' : '#f8fafc' }}>
        ...
        <div className="mt-12 pt-6 border-t border-background-200/70">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-accent-50 rounded-xl p-4">
              <div className="text-xs text-foreground-600 mb-1">Budget Évité Sprint 2</div>
              <div className="text-xl font-bold text-accent-500">{meta.totalBudgetAvoided}</div>
            </div>
            <div className="bg-primary-50 rounded-xl p-4">
              <div className="text-xs text-foreground-600 mb-1">Budget Évité Combiné (S1+S2)</div>
              <div className="text-xl font-bold text-primary-500">{meta.combinedBudgetAvoided}</div>
            </div>
            <div className="bg-secondary-50 rounded-xl p-4">
              <div className="text-xs text-foreground-600 mb-1">Valeur Créée Estimée</div>
              <div className="text-xl font-bold text-foreground-950">{meta.totalValueCreated}</div>
            </div>
