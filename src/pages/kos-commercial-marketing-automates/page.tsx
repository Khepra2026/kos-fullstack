...
import { useCommercialMarketingAutomates } from '@/hooks/useCommercialMarketingAutomates';
import type { KOSCommercialMarketingAutomate } from '@/hooks/useCommercialMarketingAutomates';
...
export default function KOSCommercialMarketingAutomatesPage() {
  const { i18n } = useTranslation();
  const lang = i18n.language.startsWith('en') ? 'en' : 'fr';
  const { automates, categories, kpis, isLive, loading, error, refetch } = useCommercialMarketingAutomates();
  ...
  const coreCategories = ['prospection-generation', 'marketing-digital-inbound', 'campagnes-automatisation', 'analytics-performance'];

  const funnelChain = [
    { id: 'prospection-generation', label: 'Prospection', icon: 'ri-radar-line', color: '#EA580C', desc: 'Identification des comptes cibles et signaux d\'achat' },
    { id: 'marketing-digital-inbound', label: 'Inbound', icon: 'ri-megaphone-line', color: '#0D7B5F', desc: 'Content marketing, SEO et réseaux sociaux' },
    { id: 'campagnes-automatisation', label: 'Campagnes', icon: 'ri-mail-send-line', color: '#BE123C', desc: 'Automatisation multicanal et nurturing' },
    { id: 'branding-positionnement', label: 'Branding', icon: 'ri-palette-line', color: '#9B7B2C', desc: 'Positionnement marque et thought leadership' },
    { id: 'partenariats-alliances', label: 'Partenariats', icon: 'ri-hand-heart-line', color: '#86BC25', desc: 'Alliances stratégiques et channel partners' },
    { id: 'evenements-relations-publiques', label: 'Événements', icon: 'ri-calendar-event-line', color: '#DC2626', desc: 'Webinaires, conférences et relations presse' },
    { id: 'sales-enablement-outils', label: 'Sales Enable', icon: 'ri-tools-line', color: '#5B8C2A', desc: 'Kits commerciaux, battle cards et formation' },
    { id: 'analytics-performance', label: 'Analytics', icon: 'ri-line-chart-line', color: '#8B3040', desc: 'Pipeline, attribution et dashboards exécutifs' },
  ];

  return (
    <KOSHubLayout hubId={59}>
      <SeoHead
        title="KOS Développement Commercial & Marketing™ — Automates Prospection & Croissance | KHEPRA EXPERTS"
        description="24 automates développement commercial et marketing KOS : prospection, inbound, campagnes, branding, partenariats, événements, sales enablement, analytics. 72K+ leads, 34.8M€ revenue influencé."
        keywords="KOS développement commercial, marketing automatisé, prospection B2B, campagnes marketing, branding, sales enablement, KHEPRA EXPERTS"
        canonicalPath="/kos-commercial-marketing-automates"
        ogType="website"
      ...
      <section className="relative bg-background-100 border-b border-background-200/70">
        <div className="absolute inset-0">
          <img
            src="https://readdy.ai/api/search-image?query=Abstract%20business%20growth%20and%20marketing%20automation%20atmosphere%20with%20warm%20orange%20and%20emerald%20tones%2C%20elegant%20geometric%20patterns%20suggesting%20lead%20generation%20funnels%20and%20revenue%20growth%20visualization%2C%20sophisticated%20B2B%20marketing%20aesthetic%20with%20rich%20textures%2C%20premium%20business%20development%20ambiance%2C%20no%20text%20no%20human%20figures%2C%20modern%20consulting%20marketing%20aesthetic%20with%20interconnected%20network%20nodes%20and%20growth%20arrow%20motifs&width=1920&height=600&seq=kos-commercial-marketing-hero&orientation=landscape"
            alt=""
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
              Le KOS ne se contente pas d'exécuter.
              <span className="block text-orange-400 mt-2">Il trouve les clients, les attire, les convertit.</span>
            </h1>
            <p className="text-lg sm:text-xl text-gray-300 leading-relaxed mb-8 max-w-3xl mx-auto">
              <strong className="text-white">{kpis.total_agents} automates</strong> dédiés au développement commercial et marketing sur <strong className="text-white">{kpis.categories} domaines</strong>.{' '}
              <strong className="text-white">{formatNumber(kpis.total_leads_generated)}+ leads générés</strong>,{' '}
              <strong className="text-white">{formatNumber(kpis.total_campaigns_run)}+ campagnes</strong> pour{' '}
              <strong className="text-white">{formatCurrency(kpis.total_revenue_influenced)}</strong> de revenu influencé.
            </p>
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-orange-500/20 border border-orange-400/30 backdrop-blur-sm">
                <i className="ri-funds-line text-orange-400" />
                <span className="text-sm text-orange-300 font-semibold">ROI {kpis.avg_roi}x</span>
              </div>
              <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/20 border border-emerald-400/30 backdrop-blur-sm">
      ...
      <section className="py-4 bg-foreground-950 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-10 gap-3">
            {[
              { label: 'Total', value: String(kpis.total_agents), icon: 'ri-robot-line', color: '#EA580C' },
              ...
              { label: 'Leads générés', value: formatNumber(kpis.total_leads_generated), icon: 'ri-user-search-line', color: '#0D7B5F' },
              { label: 'Campagnes', value: formatNumber(kpis.total_campaigns_run), icon: 'ri-rocket-line', color: '#BE123C' },
              { label: 'Taux conv.', value: kpis.overall_conversion_rate + '%', icon: 'ri-line-chart-line', color: '#DC2626' },
              { label: 'Revenue influencé', value: formatCurrency(kpis.total_revenue_influenced), icon: 'ri-funds-line', color: '#0D7B5F' },
              { label: 'ROI moyen', value: kpis.avg_roi + 'x', icon: 'ri-arrow-up-line', color: '#86BC25' },
              { label: 'Canaux', value: String(kpis.channels), icon: 'ri-share-forward-line', color: '#E8C547' },
            ].map((stat, i) => (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-foreground-950 mb-2">
              Chaîne Commerciale & Marketing — De la Prospection au ROI
            </h2>
            <p className="text-foreground-600">Un parcours intégré en 8 étapes qui transforme le marché UEMOA/CEMAC en pipeline qualifié.</p>
          </div>

          <div className="relative">
            <div className="hidden lg:block absolute top-12 left-[8%] right-[8%] h-0.5 bg-background-200" />
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
              {funnelChain.map((step, idx) => {
      ...
      <section className="py-8 sm:py-10 bg-background-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-foreground-950 mb-2">
              8 Domaines — Le Moteur de Croissance Commerciale du KOS
            </h2>
            <p className="text-foreground-600">De la prospection à l'analyse du ROI, chaque domaine couvre une fonction clé du développement commercial.</p>
          </div>
                ...
                >
                  {isCore && (
                    <div className="absolute top-3 right-3">
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-orange-100 text-orange-700 font-bold border border-orange-200">PILIER</span>
                    </div>
                  )}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {loading && (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              <div className="w-12 h-12 border-4 border-background-200 border-t-orange-500 rounded-full animate-spin" />
              <p className="text-sm text-foreground-500">Chargement des 24 automates commercial & marketing KOS...</p>
            </div>
          )}
          ...
          {!loading && !error && (
            <>
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs text-foreground-400">{formatNumber(kpis.total_leads_generated)} leads générés</span>
              </div>
              ...
              {filteredAutomates.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  {filteredAutomates.map((automate: KOSCommercialMarketingAutomate) => {
                        ...
                        >
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {(automate.leads_generated > 0 || automate.revenue_influenced > 0) && (
                                <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs">
                                  {automate.leads_generated > 0 && (
                                    <span className="text-orange-700 font-bold">{formatNumber(automate.leads_generated)} leads</span>
                                  )}
                                  {automate.revenue_influenced > 0 && (
                                    <>
                                      {automate.leads_generated > 0 && <span className="text-foreground-300">·</span>}
                                      <span className="text-emerald-700 font-bold">{formatCurrency(automate.revenue_influenced)}</span>
                                    </>
                                  )}
                                </div>
                              )}
                        ...
                        {isExpanded && (
                          <div className="px-5 pb-5 border-t border-background-200 pt-4">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                              <div>
                                ...
                                <div className="mt-3 flex flex-wrap gap-1.5">
                                  {automate.channels.map(ch => (
                                    <span key={ch} className="text-[10px] px-2 py-0.5 rounded-full bg-orange-50 border border-orange-200 text-orange-700 font-medium">{ch}</span>
                                  ))}
                                </div>
                                {automate.roi_avg > 0 && (
                                  <div className="mt-2 flex items-center gap-1.5">
                                    <span className="text-[10px] text-foreground-400">ROI :</span>
                                    <span className="text-xs font-bold text-emerald-700">{automate.roi_avg}x</span>
                                  </div>
                                )}
                              </div>
                            ...
                            <div className="mt-3 flex items-center gap-4 text-[10px] text-foreground-400 pt-3 border-t border-background-100">
                              ...
                              {automate.campaigns_run > 0 && (
                                <span><i className="ri-rocket-line mr-1" />{automate.campaigns_run} campagnes</span>
                              )}
      ...
      <section className="py-12 sm:py-16 bg-white border-t border-background-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-foreground-950 mb-2">
              Écosystème KOS — Commercial & Marketing
            </h2>
            <p className="text-foreground-600">Accès rapide aux modules connexes de l'écosystème commercial KOS.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Commercial & Marketing', path: '/kos-commercial-marketing-automates', icon: 'ri-rocket-line', color: '#EA580C', current: true },
              { label: 'Référents Métiers', path: '/kos-referents-metiers-automates', icon: 'ri-robot-line', color: '#5B8C2A' },
              ...
              { label: 'Market Intelligence', path: '/kos-market-intelligence-command', icon: 'ri-bar-chart-grouped-line', color: '#DC2626' },
              { label: 'Lead Scoring Command', path: '/kos-lead-scoring-command', icon: 'ri-fire-line', color: '#EA580C' },
              { label: 'Blog Writing Automates', path: '/kos-blog-writing-automates', icon: 'ri-article-line', color: '#0D7B5F' },
              { label: 'Social Media Command', path: '/kos-social-media-command', icon: 'ri-share-forward-line', color: '#9B7B2C' },
            ].map(link => (
              <a key={link.path} href={link.path} className={`rounded-xl border p-4 text-center hover:shadow-md transition-all cursor-pointer block ${
                link.current ? 'border-orange-300 bg-orange-50/40 ring-2 ring-orange-400' : 'border-background-200 bg-background-50 hover:border-foreground-200'
              }`}>
                ...
                {link.current && (
                  <span className="block text-[10px] text-orange-700 font-bold mt-1">Vous êtes ici</span>
                )}
              </a>
            ))}
          </div>
