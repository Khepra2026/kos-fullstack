...
import { ARTICLE_VERROU_DATA, ARTICLE_VERROU_CONTENT } from './data.tsx';
import { buildArticleHreflang } from '@/utils/hreflang';
import { useTranslation } from 'react-i18next';

const SITE_URL = import.meta.env.VITE_SITE_URL || 'https://khepraexperts.com';
const ARTICLE_URL = `${SITE_URL}${ARTICLE_VERROU_DATA.meta.canonicalPath}`;

function buildArticleSchema(d: typeof ARTICLE_VERROU_DATA, isEn: boolean) {
  return {
...
function buildFaqSchema(d: typeof ARTICLE_VERROU_DATA, isEn: boolean) {
  return {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
...
function FrictionCard({
  ...
  f: typeof ARTICLE_VERROU_CONTENT.pointsFriction.frictions[0];
}) {
...
function PillarCard({ pillar }: { pillar: typeof ARTICLE_VERROU_CONTENT.architectureSolution.pillars[0] }) {
  const [open, setOpen] = useState(false);
  return (
...
function MaturityDimension({ dim }: { dim: typeof ARTICLE_VERROU_CONTENT.outilPremium.dimensions[0] }) {
  const [open, setOpen] = useState(false);
  return (
...
export default function VerrouNationalitePage() {
  const navigate = useNavigate();
  const [openFaq, setOpenFaq] = useState<number | null>(null);
  const { i18n } = useTranslation();
  const isEn = i18n.language === 'en';
  const d = ARTICLE_VERROU_DATA;
  const c = ARTICLE_VERROU_CONTENT;
  ...
  return (
    <div className="min-h-screen bg-white">
      <SeoHead
        ...
        hreflangLinks={buildArticleHreflang('verrou-nationalite-competences-executives-circulaire-02-2017', isEn ? 'en' : 'fr')}
        ogUrl={ARTICLE_URL}
        ogType="article"
        ogImage={d.heroImage}
        ogImageWidth="1400"
      ...
      <div className="relative pt-20 overflow-hidden" style={{ height: '520px' }}>
        ...
        <div className="absolute bottom-8 left-0 right-0 px-6 lg:px-8 max-w-5xl mx-auto">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <RegulatoryBadge text="Circulaire n°02-2017/CB/C" />
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <main className="flex-1 min-w-0" id="main-content">
            ...
            <div className="mb-10 p-5 rounded-2xl border" style={{ background: 'rgba(201,162,39,0.04)', borderColor: 'rgba(201,162,39,0.2)' }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2">
                {[
                  ...
                  { num: 'Sujet B', label: 'B', title: "Verrou de la Nationalité (Circ. 02-2017)", href: null, current: true },
              ...
              {c.cadreReglementaire.blocks.map((block) => (
                <div key={block.id} className={`rounded-2xl border-2 ${block.border} overflow-hidden mb-6`}>
                  ...
                  <div className="px-5 sm:px-6 py-5 space-y-4">
                    {block.id === 'A' ? (
                      <>
                        <p className="text-sm text-gray-600 leading-relaxed">Le régime de la nationalité des dirigeants effectifs s'inscrit dans un corpus normatif à plusieurs niveaux :</p>
            ...
            <section id="architecture-solution" className="mb-14 scroll-mt-28">
              ...
              {/* Processus d'avis conforme visuel */}
              <div className="mb-8 rounded-2xl border border-gray-200 overflow-hidden">
                ...
                <div className="p-5">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                    {[
                      { step: '01', label: 'Dossier complet', icon: 'ri-folder-3-line', color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200' },
                      { step: '02', label: 'Transmission SG-CB-UMOA', icon: 'ri-send-plane-line', color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200' },
                      { step: '03', label: 'Examen (≤ 60 jours)', icon: 'ri-search-eye-line', color: 'text-teal-700', bg: 'bg-teal-50', border: 'border-teal-200' },
                      { step: '04', label: 'Avis conforme', icon: 'ri-check-double-line', color: 'text-indigo-700', bg: 'bg-indigo-50', border: 'border-indigo-200' },
                      { step: '05', label: 'Nomination effective', icon: 'ri-user-add-line', color: 'text-red-700', bg: 'bg-red-50', border: 'border-red-200' },
                    ].map((s, i) => (
                  ...
                  <p className="text-xs text-gray-400 italic mt-4 text-center">Toute nomination effective avant l'avis conforme constitue une infraction à l'Art. 16 de la Circulaire n°02-2017/CB/C</p>
                </div>
              </div>
                ...
                <div className="px-6 sm:px-8 py-6">
                  <div className="flex items-start gap-4 mb-4">
                    <div className="w-12 h-12 flex items-center justify-center rounded-2xl flex-shrink-0" style={{ background: 'rgba(201,162,39,0.12)', border: '1px solid rgba(201,162,39,0.25)' }}>
                      <i className="ri-passport-line text-2xl" style={{ color: '#c9a227' }}></i>
                    </div>
                    <div>
            ...
            <section id="references-officielles" className="mb-14 scroll-mt-28">
              ...
              <div className="space-y-3">
                {[
                  { title: 'Circulaire n°02-2017/CB/C relative aux conditions d\'aptitude, d\'honorabilité et de compétences des dirigeants', institution: 'Commission Bancaire de l\'UMOA', year: '2017', url: 'https://www.bceao.int' },
                  ...
                  { title: 'Convention du 24 avril 1990 portant création de la Commission Bancaire de l\'UMOA (Art. 58 — Sanctions)', institution: 'BCEAO / UEMOA', year: '1990', url: 'https://www.bceao.int' },
                  { title: 'Principes de gouvernance d\'entreprise du G20 et de l\'OCDE', institution: 'OCDE', year: '2023', url: 'https://www.oecd.org' },
                  { title: 'Basel Committee on Banking Supervision — Fit and proper assessment guidelines', institution: 'BCBS', year: '2015', url: 'https://www.bis.org' },
                ].map((ref, i) => (
              ...
              <div className="relative z-10">
                <div className="flex items-start gap-4 mb-5">
                  ...
                  <div>
                    ...
                    <h3 className="font-playfair text-xl font-bold text-white leading-snug">Sécurisez le processus de nomination de vos dirigeants non-ressortissants</h3>
                  </div>
                </div>
                <p className="text-white/70 text-sm leading-relaxed mb-6 max-w-2xl">
                  Notre mission KHEPRA VISA-Matrix™ vous fournit un score de conformité confidentiel sur 5 dimensions, un audit des dossiers de visa existants, une cartographie des conflits holding/filiale et un plan d'action de mise en conformité priorisé par niveau de risque réglementaire (Commission Bancaire UMOA).
                </p>
            ...
            <div className="sticky top-28 space-y-6">
              ...
              <div className="rounded-2xl p-5 border border-gray-200 bg-white">
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="flex items-start gap-2.5 p-2.5 rounded-lg" style={{ background: 'rgba(201,162,39,0.06)', border: '1px solid rgba(201,162,39,0.2)' }}>
                    <div className="w-6 h-6 flex items-center justify-center rounded-md flex-shrink-0" style={{ background: 'rgba(201,162,39,0.15)', color: '#c9a227', fontSize: '10px', fontWeight: 'bold' }}>B</div>
                    <div>
                      <p className="font-semibold text-xs leading-snug" style={{ color: '#c9a227' }}>Sujet B (en cours)</p>
                      <p className="text-xs text-gray-600">Verrou de la nationalité — Circ. 02-2017</p>
                    </div>
                  </div>
                  <a
              ...
              <div className="rounded-2xl p-6" style={{ background: 'linear-gradient(145deg, #050c18 0%, #0d1c2e 100%)', border: '1px solid rgba(201,162,39,0.2)' }}>
                <div className="w-10 h-10 flex items-center justify-center rounded-xl mb-4" style={{ background: 'rgba(201,162,39,0.15)' }}>
                  <i className="ri-passport-line text-xl" style={{ color: '#c9a227' }}></i>
                </div>
                <h4 className="font-playfair font-bold text-lg mb-2 leading-snug text-white">KHEPRA VISA-Matrix™</h4>
                <p className="text-sm mb-4 leading-relaxed" style={{ color: 'rgba(255,255,255,0.65)' }}>
                  Score de conformité sur 5 dimensions. Audit des visas. Plan d'action priorisé.
                </p>
                <button
                  onClick={() => navigate('/contact')}
              ...
              <div className="rounded-2xl p-5 border border-gray-200 bg-white">
                ...
                <div className="space-y-2.5">
                  {[
                    { label: 'Circulaire n°02-2017/CB/C', sub: 'Dirigeants effectifs — UMOA', icon: 'ri-file-text-line' },
                    { label: 'Circulaire n°01-2017/CB/C', sub: 'Gouvernance — UMOA', icon: 'ri-file-text-line' },
                    { label: 'Convention CB-UMOA (Art. 58)', sub: 'Sanctions disciplinaires', icon: 'ri-scales-3-line' },
                    { label: 'Principes G20/OCDE (2023)', sub: 'Gouvernance d\'entreprise', icon: 'ri-global-line' },
                    { label: 'Bâle III — Fit and proper test', sub: 'BCBS — Gouvernance bancaire', icon: 'ri-bar-chart-line' },
                  ].map((ref, i) => (
