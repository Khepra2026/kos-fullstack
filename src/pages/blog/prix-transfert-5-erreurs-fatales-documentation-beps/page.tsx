...
const article = ARTICLES_GENERATED[1];
...
export default function ArticlePrixTransfert() {
  ...
  return (
    <>
      <SeoHead
        ...
        ogType="article"
        articlePublishedTime="2026-06-17T08:00:00+00:00"
          ...
          <div className="h-[420px] md:h-[540px] w-full bg-cover bg-top relative" style={{ backgroundImage: `url(${article.hero_image_url})` }}>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="max-w-4xl mx-auto px-4 md:px-8 text-center">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6 text-sm font-bold uppercase tracking-wider" style={{ background: 'rgba(234,88,12,0.2)', border: '1px solid rgba(234,88,12,0.4)', color: '#EA580C' }}>
                  <i className="ri-file-chart-line" /> {article.category}
                </div>
        ...
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-12">
                ...
                <div className="space-y-2 mt-4">
                  {sec.highlights.map((h, j) => (
                    <div key={j} className="flex items-start gap-2 text-sm text-foreground-600 pl-12">
                      <i className="ri-arrow-right-s-line text-primary-500 flex-shrink-0 mt-0.5" />{h}
                    </div>
                  ))}
                </div>
              )}
          ...
          <section className="mb-10">
            <div className="p-6 md:p-8 rounded-2xl border-2 border-primary-500/30 bg-gradient-to-br from-background-50 to-primary-500/5">
              ...
              <div className="space-y-4">
                {article.framework.pillars.map((p, j) => {
                  const sc = getStatusColor(p.color);
                  return (
                    <div key={j} className="flex items-center gap-4 p-4 rounded-xl bg-white/70 border border-background-200">
                      <span className="text-xs font-bold text-foreground-400 w-6">{j + 1}</span>
                      ...
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${sc.bg} ${sc.text} border ${sc.border}`}>{p.status}</span>
                  </div>
                  );
                })}
              </div>
          ...
          <section className="mb-10">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {article.cas_usage.map((cas, j) => (
                <div key={j} className="p-5 rounded-2xl bg-background-100 border border-background-200">
                  <div className="w-10 h-10 rounded-xl bg-primary-500/10 flex items-center justify-center mb-3"><i className={`${cas.icon} text-primary-500 text-lg`} /></div>
                  ...
                  <div className="p-3 rounded-lg bg-amber-50 border border-amber-200"><p className="text-xs text-amber-800 font-medium"><strong>Impact :</strong> {cas.impact}</p></div>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="mb-10">
            ...
            <div className="space-y-4">
              {article.implications.map((imp, j) => (
                <div key={j} className="p-5 rounded-2xl bg-background-100 border border-background-200">
                  <h3 className="text-sm font-bold text-foreground-950 mb-2 flex items-center gap-2"><i className={`${imp.icon} text-primary-500`} />{imp.audience}</h3>
