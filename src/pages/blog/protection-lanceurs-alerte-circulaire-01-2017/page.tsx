...
import { ARTICLE_LANCEURS_DATA, ARTICLE_LANCEURS_CONTENT } from './data.tsx';
import { buildArticleHreflang } from '@/utils/hreflang';
import { useTranslation } from 'react-i18next';

const SITE_URL = import.meta.env.VITE_SITE_URL || 'https://khepraexperts.com';
const ARTICLE_URL = `${SITE_URL}${ARTICLE_LANCEURS_DATA.meta.canonicalPath}`;

function buildArticleSchema(d: typeof ARTICLE_LANCEURS_DATA, isEn: boolean) {
  return {
...
function buildFaqSchema(d: typeof ARTICLE_LANCEURS_DATA, isEn: boolean) {
  return {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
...
function FrictionCard({
  ...
  f: typeof ARTICLE_LANCEURS_CONTENT.pointsFriction.frictions[0];
}) {
...
function PillarCard({ pillar }: { pillar: typeof ARTICLE_LANCEURS_CONTENT.architectureSolution.pillars[0] }) {
  const [open, setOpen] = useState(false);
  return (
...
function MaturityDimension({ dim }: { dim: typeof ARTICLE_LANCEURS_CONTENT.outilPremium.dimensions[0] }) {
  const [open, setOpen] = useState(false);
  return (
...
export default function ProtectionLanceursAlertePage() {
  const navigate = useNavigate();
  const [openFaq, setOpenFaq] = useState<number | null>(null);
  const { i18n } = useTranslation();
  const isEn = i18n.language === 'en';
  const d = ARTICLE_LANCEURS_DATA;
  const c = ARTICLE_LANCEURS_CONTENT;
  ...
  return (
    <div className="min-h-screen bg-white">
      <SeoHead
        ...
        hreflangLinks={buildArticleHreflang('protection-lanceurs-alerte-circulaire-01-2017', isEn ? 'en' : 'fr')}
        ogUrl={ARTICLE_URL}
        ogType="article"
        ogImage={d.heroImage}
        ogImageWidth="1400"
      ...
      <div className="relative pt-20 overflow-hidden" style={{ height: '520px' }}>
        ...
        <div className="absolute bottom-8 left-0 right-0 px-6 lg:px-8 max-w-5xl mx-auto">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            <RegulatoryBadge text="Circulaire n°01-2017/CB/C — Art. 44" />
          ...
          <main className="flex-1 min-w-0" id="main-content">
            ...
            <div className="mb-10 p-5 rounded-2xl border" style={{ background: 'rgba(201,162,39,0.04)', borderColor: 'rgba(201,162,39,0.2)' }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2">
                {[
                  { num: 'Sujet A', label: 'A', title: "Indépendance des Administrateurs (Circ. 01-2017)", href: '/blog/independance-administrateurs-circulaire-01-2017/', current: false },
                  ...
                  { num: 'Sujet C', label: 'C', title: '3 Lignes de Défense (Circ. 03-2017)', href: '/blog/3-lignes-defense-circulaire-03-2017/', current: false },
                  { num: 'Sujet D', label: 'D', title: "Lanceurs d'Alerte — Art. 44", href: null, current: true },
              ...
              {c.cadreReglementaire.blocks.map((block) => (
                <div key={block.id} className={`rounded-2xl border-2 ${block.border} overflow-hidden mb-6`}>
                  ...
                  <div className="px-5 sm:px-6 py-5 space-y-4">
                    {block.id === 'A' ? (
                      <>
                        <p className="text-sm text-gray-600 leading-relaxed">La protection des lanceurs d'alerte s'inscrit dans un corpus normatif à trois niveaux : réglementaire local, réglementaire européen et standards internationaux GAFI :</p>
            ...
            <section id="architecture-solution" className="mb-14 scroll-mt-28">
              <SectionHeading id="architecture-solution" number="III" title={c.architectureSolution.heading} />
              <p className="text-sm text-gray-600 leading-relaxed mb-8">{c.architectureSolution.intro}</p>

              {/* Processus de traitement d'un signalement visuel */}
              <div className="mb-8 rounded-2xl border border-gray-200 overflow-hidden">
                <div className="px-5 py-4 bg-gray-50 border-b border-gray-200">
                  <p className="text-sm font-bold text-gray-700 flex items-center gap-2">
                    <i className="ri-git-pull-request-line" style={{ color: '#c9a227' }}></i>
                ...
                <div className="p-5">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                    {[
                      { step: '01', label: 'Signalement', icon: 'ri-shield-user-line', color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200' },
                      { step: '02', label: 'Accusé de réception', icon: 'ri-check-double-line', color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200' },
                      { step: '03', label: 'Enquête interne', icon: 'ri-search-eye-line', color: 'text-teal-700', bg: 'bg-teal-50', border: 'border-teal-200' },
                      { step: '04', label: 'Décision', icon: 'ri-scales-3-line', color: 'text-indigo-700', bg: 'bg-indigo-50', border: 'border-indigo-200' },
                      { step: '05', label: 'Protection & Suivi', icon: 'ri-user-forbid-line', color: 'text-red-700', bg: 'bg-red-50', border: 'border-red-200' },
                    ].map((s, i) => (
                      <div key={i} className="flex sm:flex-col items-center gap-2 flex-1 min-w-0">
                        <div className={`w-10 h-10 flex items-center justify-center rounded-xl border-2 ${s.border} ${s.bg} flex-shrink-0`}>
                          <i className={`${s.icon} ${s.color} text-base`}></i>
                        </div>
                        <div className="text-center min-w-0">
                          <p className={`text-xs font-bold ${s.color} uppercase tracking-wide`}>{s.step}</p>
                          <p className="text-xs text-gray-600 leading-snug">{s.label}</p>
                        </div>
                        {i < 4 && (
                          <div className="hidden sm:flex items-center justify-center flex-shrink-0 w-4">
                  ...
                  <p className="text-xs text-gray-400 italic mt-4 text-center">Délai d'accusé de réception : 7 jours. Délai d'enquête : 3 mois. Protection contre les représailles : 2 ans.</p>
                </div>
              </div>
            ...
            <section id="distinction-obligation" className="mb-14 scroll-mt-28">
              <SectionHeading id="distinction-obligation" number="IV" title={c.distinctionObligation.heading} />
            ...
            <section id="geo-direct-answers" className="mb-14 scroll-mt-28">
              <SectionHeading id="geo-direct-answers" number="VII" title="Réponses Directes — Optimisation IA Générative" />
            ...
            <section id="references-officielles" className="mb-14 scroll-mt-28">
              ...
              <div className="space-y-3">
                {[
                  { title: 'Circulaire n°01-2017/CB/C relative à la gouvernance des établissements de crédit (Art. 44)', institution: 'Commission Bancaire de l\'UMOA', year: '2017', url: 'https://www.bceao.int' },
                  { title: 'Directive (UE) 2019/1937 du Parlement européen et du Conseil sur la protection des personnes qui signalent des violations', institution: 'Union Européenne', year: '2019', url: 'https://eur-lex.europa.eu' },
                  { title: 'Recommandations du GAFI — Normes internationales sur la lutte contre le blanchiment de capitaux (Rec. 20-22)', institution: 'GAFI / FATF', year: '2024', url: 'https://www.fatf-gafi.org' },
                  { title: 'Directive UEMOA n°02/2015/CM/UEMOA relative à la lutte contre le blanchiment de capitaux et le financement du terrorisme', institution: 'UEMOA', year: '2015', url: 'https://www.uemoa.int' },
                  { title: 'Convention du 24 avril 1990 portant création de la Commission Bancaire de l\'UMOA (Art. 58)', institution: 'BCEAO / UEMOA', year: '1990', url: 'https://www.bceao.int' },
                ].map((ref, i) => (
              ...
              <div className="relative z-10">
                <div className="flex items-start gap-4 mb-5">
                  ...
                  <div>
                    ...
                    <h3 className="font-playfair text-xl font-bold text-white leading-snug">Certifiez la conformité du dispositif de protection des lanceurs d'alerte de votre institution</h3>
                  </div>
                </div>
                <p className="text-white/70 text-sm leading-relaxed mb-6 max-w-2xl">
                  Notre mission KHEPRA WHISTLEBLOWER-Matrix™ vous fournit un score de conformité confidentiel sur 5 dimensions, un audit des canaux de signalement, une cartographie des conflits de corpus normatifs et un plan d'action de mise en conformité priorisé par niveau de risque réglementaire et de réputation.
                </p>
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  >
                    Sujet A — Indépendance des Administrateurs
                    <i className="ri-arrow-right-line"></i>
                  </button>
                  <button
          ...
          <aside className="lg:w-80 flex-shrink-0 space-y-6">
            <div className="sticky top-28 space-y-6">
              ...
              <div className="rounded-2xl p-5 border border-gray-200 bg-white">
                ...
                <div className="space-y-2.5">
                  <a
                    ...
                    onClick={(e) => { e.preventDefault(); navigate('/blog/independance-administrateurs-circulaire-01-2017/'); }}
                  ...
                  >
                    <div className="w-6 h-6 flex items-center justify-center rounded-md flex-shrink-0 bg-white border border-gray-200 text-gray-500" style={{ fontSize: '10px', fontWeight: 'bold' }}>A</div>
                    <div>
                      <p className="font-semibold text-xs text-gray-700 group-hover:text-yellow-700 leading-snug">Sujet A</p>
                      <p className="text-xs text-gray-500">Indépendance des administrateurs — Circ. 01-2017</p>
                    </div>
                  </a>
                  <a
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Verrou de la nationalité — Circ. 02-2017</p>
                    </div>
                  </a>
                  <a
                  ...
                  <div className="flex items-start gap-2.5 p-2.5 rounded-lg" style={{ background: 'rgba(201,162,39,0.06)', border: '1px solid rgba(201,162,39,0.2)' }}>
                    <div className="w-6 h-6 flex items-center justify-center rounded-md flex-shrink-0" style={{ background: 'rgba(201,162,39,0.15)', color: '#c9a227', fontSize: '10px', fontWeight: 'bold' }}>D</div>
                    <div>
                      <p className="font-semibold text-xs leading-snug" style={{ color: '#c9a227' }}>Sujet D (en cours)</p>
                      <p className="text-xs text-gray-600">Lanceurs d'Alerte — Art. 44, Circ. 01-2017</p>
                    </div>
                  </div>
                  <a
              ...
              <div className="rounded-2xl p-6" style={{ background: 'linear-gradient(145deg, #050c18 0%, #0d1c2e 100%)', border: '1px solid rgba(201,162,39,0.2)' }}>
                ...
                <h4 className="font-playfair font-bold text-lg mb-2 leading-snug text-white">KHEPRA WHISTLEBLOWER-Matrix™</h4>
                <p className="text-sm mb-4 leading-relaxed" style={{ color: 'rgba(255,255,255,0.65)' }}>
                  Score de conformité sur 5 dimensions. Audit des canaux. Plan d'action priorisé.
                </p>
                <button
                  onClick={() => navigate('/contact')}
              ...
              <div className="rounded-2xl p-5 border border-gray-200 bg-white">
                ...
                <div className="space-y-2.5">
                  {[
                    { label: 'Circulaire n°01-2017/CB/C', sub: "Art. 44 — Lanceurs d'alerte — UMOA", icon: 'ri-file-text-line' },
                    { label: 'Directive (UE) 2019/1937', sub: "Protection des donneurs d'alerte", icon: 'ri-global-line' },
                    { label: 'GAFI Recommandations 20-22', sub: 'LBC/FT — Protection collaborateurs', icon: 'ri-shield-check-line' },
                    { label: 'Circulaire n°03-2017/CB/C', sub: 'Contrôle interne — UMOA', icon: 'ri-file-text-line' },
                    { label: 'Convention du 24 avril 1990', sub: 'Art. 58 — Sanctions disciplinaires', icon: 'ri-scales-3-line' },
                  ].map((ref, i) => (
