...
export class KOSGraph {
  ...
  getNode(id: string): KOSNode | undefined {
  ...
  getNodesByType(type: string): KOSNode[] {
  ...
  getEdges(source?: string): KOSEdge[] {
    const result: KOSEdge[] = [];
  ...
  getSuccessors(nodeId: string): KOSNode[] {
    const edges = this.adjacency.get(nodeId) || [];
    return edges
      .map(e => this.nodes.get(e.target))
      .filter((n): n is KOSNode => n !== undefined);
  }

  /** Get successors with relation info */
  getSuccessorsWithRelations(nodeId: string): Array<{ node: KOSNode; relation: string; weight: number }> {
    const edges = this.adjacency.get(nodeId) || [];
    return edges
      .map(e => ({ node: this.nodes.get(e.target), relation: e.relation, weight: e.weight }))
      .filter((r): r is { node: KOSNode; relation: string; weight: number } => r.node !== undefined);
  }

  /** Get predecessors (incoming neighbors) */
  getPredecessors(nodeId: string): KOSNode[] {
  ...
  findRelated(nodeId: string, maxHops: number = 2, relationFilter?: string[]): Array<{ node: KOSNode; hops: number; path: string[] }> {
    const visited = new Set<string>([nodeId]);
    const queue: Array<{ id: string; hops: number; path: string[] }> = [{ id: nodeId, hops: 0, path: [nodeId] }];
    const result: Array<{ node: KOSNode; hops: number; path: string[] }> = [];
  ...
  findRelatedDocuments(themeId: string, maxHops: number = 2): KOSNode[] {
  ...
  toJSON(): { nodes: KOSNode[]; edges: KOSEdge[] } {
    return {
  ...
  fromJSON(data: { nodes: KOSNode[]; edges: KOSEdge[] }): void {
    this.nodes.clear();
  ...
  static fromJSON(data: { nodes: KOSNode[]; edges: KOSEdge[] }): KOSGraph {
    const graph = new KOSGraph();
    graph.fromJSON(data);
    return graph;
  }
...
let globalKOSInstance: KOSGraph | null = null;

export function getGlobalKOSGraph(): KOSGraph {
  if (!globalKOSInstance) {
    globalKOSInstance = new KOSGraph();
  }
