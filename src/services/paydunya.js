...
export function getPayDunyaSetup() {
  // Compatibilité avec ton ancien code - retourne config, pas d'objet paydunya
  return {
...
export async function createCheckoutInvoice({ amount, description, items, customData }) {
  ...
  if (!res.ok || (json.response_code !== "00" && json.response_code !== "200" && json.status !== "success")) {
    throw new Error(json.response_text || json.message || `PayDunya error ${res.status}`);
  }
