// ═══════════════════════════════════════════════════════════════════════════
// KOSRAGSearchBar — Barre de recherche RAG enrichie par le KOS Graph
// Affiche les thèmes détectés, les voisins du graphe, et les résultats RAG v9
// ═══════════════════════════════════════════════════════════════════════════

import { useState, useCallback, useRef } from 'react';
import { useKOSRAG, type KOSRAGResult, type KOSEnrichedNeighbor } from '@/hooks/useKOSRAG';

export default function KOSRAGSearchBar() {
  const [query, setQuery] = useState('');
  const [result, setResult] = useState<KOSRAGResult | null>(null);
  const [kosHops, setKosHops] = useState(2);
  ...
  const handleSearch = useCallback(async (searchQuery: string) => {
    ...
    const res = await searchKOS(searchQuery, { lang: 'fr', enrichKOS: true, kosHops });
    setResult(res);
  }, [searchKOS, kosHops]);
          ...
          {result.kos_enrichment.active && result.kos_enrichment.voisins_enrichis.length > 0 && (
            <div className="bg-accent-50/60 border border-accent-200/60 rounded-xl p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[180px] overflow-y-auto">
                {result.kos_enrichment.voisins_enrichis.slice(0, 8).map((neighbor: KOSEnrichedNeighbor, idx: number) => (
