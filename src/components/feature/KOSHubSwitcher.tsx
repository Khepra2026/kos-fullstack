...
import { kosHubs } from '@/mocks/kosDashboard';
import KOSNotificationBell from '@/components/feature/KOSNotificationBell';

interface KOSHubSwitcherProps {
  currentHubId: number;
  activeTab?: string;
  tabLabel?: string;
}
...
export default function KOSHubSwitcher({ currentHubId, activeTab, tabLabel }: KOSHubSwitcherProps) {
  const [showDropdown, setShowDropdown] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);

  const currentIndex = kosHubs.findIndex(h => h.id === currentHubId);
  const currentHub = kosHubs[currentIndex];
  const totalHubs = kosHubs.length;

  const prevIndex = currentIndex > 0 ? currentIndex - 1 : totalHubs - 1;
  const nextIndex = currentIndex < totalHubs - 1 ? currentIndex + 1 : 0;
  const prevHub = kosHubs[prevIndex];
  const nextHub = kosHubs[nextIndex];

  const phasesWithHubs = kosHubs.reduce<Record<string, typeof kosHubs>>((acc, hub) => {
