...
import { useKOSNotifications, KOSCriticalEvent } from '@/hooks/useKOSNotifications';

function isCrossResolutionEvent(event: KOSCriticalEvent): boolean {
  return (event.metric_name === 'cross_resolution' || (event.title && event.title.startsWith('Résolution croisée')));
}

export default function KOSNotificationBell() {
        ...
        <div className="absolute right-0 top-10 z-50 w-96 max-h-[480px] overflow-y-auto bg-background-50 border border-background-200/70 rounded-xl shadow-lg">
          ...
          {events.length === 0 ? (
            ...
            <div className="divide-y divide-background-200/50">
              ...
              {crossResolutionEvents.slice(0, 5).map((event: KOSCriticalEvent) => {
              ...
              {/* Regular events */}
              {regularEvents.slice(0, 20).map((event: KOSCriticalEvent) => {
