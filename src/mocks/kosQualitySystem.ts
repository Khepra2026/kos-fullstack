...
export interface KOSDetectedError {
  id: string;
...
export interface KOSCorrectiveAction {
  id: string;
  description: string;
  agent: string;
...
export interface KOSAgent {
  ...
  lastScan: string;
  kpis: KOSAgentKPI[];
  errors: KOSDetectedError[];
  actions: KOSCorrectiveAction[];
}

export interface KOSGlobalReport {
  generatedAt: string;
  globalScore: number;
  targetScore: number;
  totalErrors: number;
...
export const KOS_GLOBAL_REPORT: KOSGlobalReport = {
...
export const KOS_AGENTS: KOSAgent[] = [
  {
    id: 'url-integrity',
    number: '01',
