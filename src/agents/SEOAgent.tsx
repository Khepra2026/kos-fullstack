...
import { KOSContent, SEOData } from '@/types/kos';

function generateTitle(content: Partial<KOSContent>, maxLength: number): string {
  return (content.title || 'Article Khepra').slice(0, maxLength);
}

function generateMeta(content: Partial<KOSContent>, maxLength: number): string {
...
async function extractKeywords(content: Partial<KOSContent>): Promise<string[]> {
...
async function generateFAQ(content: Partial<KOSContent>): Promise<{ q: string; a: string }[]> {
  return [
...
function buildSchemaOrg(content: Partial<KOSContent>): Record<string, unknown> {
  return {
...
function buildOpenGraph(content: Partial<KOSContent>): Record<string, unknown> {
  return {
    title: content.title || '',
    description: content.summary || '',
    type: 'article',
...
export function useSEOAgent() {
  ...
  const optimize = async (content: Partial<KOSContent>): Promise<SEOData> => {
    const seo: SEOData = {
  ...
  const findRelevantPages = async (content: Partial<KOSContent>, sitemap: { url: string; title: string }[]) => {
    try {
