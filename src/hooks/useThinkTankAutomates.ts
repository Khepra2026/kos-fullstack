...
import { KOS_THINK_TANK_AUTOMATES as mockAutomates, THINK_TANK_CATEGORIES as mockCategories, THINK_TANK_AUTOMATES_KPIS as mockKPIs } from '@/mocks/thinkTankAutomates';
import type { thinkTankAutomate, ThinkTankAutomateCategory } from '@/mocks/thinkTankAutomates';

export type { thinkTankAutomate, ThinkTankAutomateCategory };
...
function mapAutomate(row: Record<string, unknown>): thinkTankAutomate {
  return {
    ...
    priority: row.priority as thinkTankAutomate['priority'],
    kpis: Array.isArray(row.kpis) ? row.kpis as thinkTankAutomate['kpis'] : [],
  };
}
