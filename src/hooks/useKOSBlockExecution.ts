...
} from '@/mocks/kosBlockExecution';
import type {
  KOSBlockFamily,
  KOSBlockExecutionGlobal,
  KOSBlockExecutionPhase,
  KOSBlockExecutionLog,
  KOSBlockTask,
} from '@/mocks/kosBlockExecution';
import {
...
export interface KOSBlockExecutionData {
  families: KOSBlockFamily[];
  global: KOSBlockExecutionGlobal;
  phases: KOSBlockExecutionPhase[];
  logs: KOSBlockExecutionLog[];
  tasks: KOSBlockTask[];
  isLive: boolean;
...
export function useKOSBlockExecution() {
  const [data, setData] = useState<KOSBlockExecutionData>({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const liveFamilies: KOSBlockFamily[] = mockFamilies.map(mock => {
      ...
      const liveGlobal: KOSBlockExecutionGlobal = {
        ...mockGlobal,
    ...
    for (let p = 0; p < totalPhases; p++) {
      ...
      for (const familyId of phase.families) {
        ...
        for (let a = 0; a < countToDeploy; a++) {
          ...
          const inProgressLog: KOSBlockExecutionLog = {
          ...
          if (data.isLive && familyTable) {
            try {
              ...
              if (updateError) {
                const errorLog: KOSBlockExecutionLog = {
              ...
              const completedLog: KOSBlockExecutionLog = {
        ...
        if (countToDeploy < family.tasks_pending) {
          ...
          for (let r = 0; r < remaining; r++) {
            ...
            const simLog: KOSBlockExecutionLog = {
  ...
  const startFullSpectrumExecution = useCallback(async () => {
    ...
    const correctionPromise = (async () => {
      ...
      for (let i = 0; i < pendingCorrections.length; i += batchSize) {
        ...
        const batchLog: KOSBlockExecutionLog = {
  ...
  const setLogs = useCallback((updater: (prev: KOSBlockExecutionLog[]) => KOSBlockExecutionLog[]) => {
