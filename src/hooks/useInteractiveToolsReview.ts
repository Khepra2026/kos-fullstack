...
import { KOS_INTERACTIVE_TOOLS_REVIEW_AUTOMATES as mockAutomates, INTERACTIVE_TOOLS_REVIEW_CATEGORIES as mockCategories, INTERACTIVE_TOOLS_REVIEW_KPIS as mockKPIs } from '@/mocks/kosInteractiveToolsReview';
import type { KOSInteractiveToolsReviewAutomate, InteractiveToolsReviewCategory } from '@/mocks/kosInteractiveToolsReview';

export type { KOSInteractiveToolsReviewAutomate, InteractiveToolsReviewCategory };
...
export interface InteractiveToolsReviewData {
  automates: KOSInteractiveToolsReviewAutomate[];
  categories: InteractiveToolsReviewCategory[];
  kpis: InteractiveToolsReviewKPIs;
  isLive: boolean;
}

function mapAutomate(row: Record<string, unknown>): KOSInteractiveToolsReviewAutomate {
  return {
    id: row.id as string,
    name: row.name as string,
    category: row.category as string,
    tech_stack: Array.isArray(row.tech_stack) ? row.tech_stack as string[] : [],
    status: row.status as KOSInteractiveToolsReviewAutomate['status'],
    ...
    priority: row.priority as KOSInteractiveToolsReviewAutomate['priority'],
    kpis: Array.isArray(row.kpis) ? row.kpis as KOSInteractiveToolsReviewAutomate['kpis'] : [],
  };
}
