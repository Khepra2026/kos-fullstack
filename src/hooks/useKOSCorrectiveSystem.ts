...
export function useKOSCorrectiveSystem(): UseKOSCorrectiveSystemReturn {
  const [gaps, setGaps] = useState<gapRegister[]>(CAS_GAP_REGISTER_MOCK);
  const [migrations, setMigrations] = useState<migrationLog[]>(CAS_MIGRATION_LOG_MOCK);
  const [actions, setActions] = useState<cASAction[]>(CAS_ACTIONS_MOCK);
  const [compliance, setCompliance] = useState<complianceImpact[]>(CAS_COMPLIANCE_IMPACT_MOCK);
  const [health, setHealth] = useState<cASSystemHealth>(CAS_SYSTEM_HEALTH_MOCK);
    ...
    try {
      ...
      if (gapsRes.data && gapsRes.data.length > 0) {
        const mapped = gapsRes.data.map((row: Record<string, unknown>) => ({
          ...
          category: row.category as gapRegister['category'],
          criticality: row.criticality as gapRegister['criticality'],
          ...
          strategy: row.strategy as gapRegister['strategy'],
      ...
      if (migrationsRes.data && migrationsRes.data.length > 0) {
        const mapped = migrationsRes.data.map((row: Record<string, unknown>) => ({
          ...
          source_type: row.source_type as migrationLog['source_type'],
      ...
      if (actionsRes.data && actionsRes.data.length > 0) {
        const mapped = actionsRes.data.map((row: Record<string, unknown>) => ({
          ...
          gap_id: row.gap_id as string,
          action_type: row.action_type as cASAction['action_type'],
          criticality: row.criticality as cASAction['criticality'],
          description: row.description as string,
          ...
          horizon: row.horizon as cASAction['horizon'],
          status: row.status as string,
          created_at: row.created_at as string,
        }));
        aData = mapped as cASAction[];
      ...
      if (complianceRes.data && complianceRes.data.length > 0) {
        ...
        cData = mapped as complianceImpact[];
