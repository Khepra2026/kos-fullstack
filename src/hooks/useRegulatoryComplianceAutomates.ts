...
import { KOS_REGULATORY_COMPLIANCE_AUTOMATES as mockAutomates, REGULATORY_COMPLIANCE_CATEGORIES as mockCategories, REGULATORY_COMPLIANCE_AUTOMATES_KPIS as mockKPIs } from '@/mocks/kosRegulatoryComplianceAutomates';
import type { KOSRegulatoryComplianceAutomate, RegulatoryComplianceAutomateCategory } from '@/mocks/kosRegulatoryComplianceAutomates';

export type { KOSRegulatoryComplianceAutomate, RegulatoryComplianceAutomateCategory };
...
export interface RegulatoryComplianceAutomatesData {
  automates: KOSRegulatoryComplianceAutomate[];
  categories: RegulatoryComplianceAutomateCategory[];
  kpis: RegulatoryComplianceAutomatesKPIs;
  isLive: boolean;
}

function mapAutomate(row: Record<string, unknown>): KOSRegulatoryComplianceAutomate {
  return {
    id: row.id as string,
    name: row.name as string,
    category: row.category as string,
    tech_stack: Array.isArray(row.tech_stack) ? row.tech_stack as string[] : [],
    status: row.status as KOSRegulatoryComplianceAutomate['status'],
    ...
    priority: row.priority as KOSRegulatoryComplianceAutomate['priority'],
    kpis: Array.isArray(row.kpis) ? row.kpis as KOSRegulatoryComplianceAutomate['kpis'] : [],
  };
}
