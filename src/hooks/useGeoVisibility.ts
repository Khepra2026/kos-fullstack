...
export function useGeoVisibility() {
  const [data, setData] = useState<GeoVisibilityData>(() => ({
    overallScore: kosAIVisibilityCommand.overallScore,
    lastScan: kosAIVisibilityCommand.lastScan,
    geoReadiness: kosAIVisibilityCommand.geoReadiness,
    llmsStatus: kosAIVisibilityCommand.llmsStatus,
    geoOptimizations: kosAIVisibilityCommand.geoOptimizations,
    scanHistory: kosAIVisibilityCommand.scanHistory,
    crawlerStatus: kosAIVisibilityCommand.crawlerStatus,
    schemaCoverage: kosAIVisibilityCommand.schemaCoverage,
    recentOperations: kosAIVisibilityCommand.recentOperations.map(op => ({
      id: op.id,
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (geoScans && geoScans.length > 0) {
        ...
        setData({
          overallScore: latestScan.score || kosAIVisibilityCommand.overallScore,
          lastScan: latestScan.created_at,
          geoReadiness: readiness,
          llmsStatus: kosAIVisibilityCommand.llmsStatus,
          geoOptimizations: kosAIVisibilityCommand.geoOptimizations,
          scanHistory: kosAIVisibilityCommand.scanHistory,
          crawlerStatus: crawlerStatus.length > 0 ? crawlerStatus : kosAIVisibilityCommand.crawlerStatus,
          schemaCoverage: schemaCoverage.length > 0 ? schemaCoverage : kosAIVisibilityCommand.schemaCoverage,
          recentOperations: recentOperations.length > 0 ? recentOperations : kosAIVisibilityCommand.recentOperations.map(op => ({
            id: op.id,
            timestamp: op.timestamp,
            type: op.type,
            target: op.target,
