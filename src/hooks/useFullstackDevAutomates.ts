...
import { KOS_DEV_AUTOMATES as mockAutomates, DEV_AUTOMATE_CATEGORIES as mockCategories, DEV_AUTOMATES_KPIS as mockKPIs } from '@/mocks/kosFullstackDevAutomates';
import type { KOSDevAutomate, DevAutomateCategory } from '@/mocks/kosFullstackDevAutomates';

export type { KOSDevAutomate, DevAutomateCategory };
...
export interface FullstackDevAutomatesData {
  automates: KOSDevAutomate[];
  categories: DevAutomateCategory[];
  kpis: DevAutomatesKPIs;
  isLive: boolean;
}

function mapAutomate(row: Record<string, unknown>): KOSDevAutomate {
  return {
    id: row.id as string,
    name: row.name as string,
    category: row.category as string,
    tech_stack: (row.tech_stack as string[]) || [],
    status: row.status as KOSDevAutomate['status'],
    ...
    priority: row.priority as KOSDevAutomate['priority'],
    kpis: (row.kpis as KOSDevAutomate['kpis']) || [],
  };
}
