...
import {
  P0_EXECUTION_BLOCKS,
  P0_GLOBAL_MILESTONES,
  P0_TIMELINE,
  P0_EXECUTIVE_SUMMARY,
  computeP0ExecutionKPIs,
} from '@/mocks/kosP0Execution';
import type { P0ExecutionBlock } from '@/mocks/kosP0Execution';

export type P0ViewTab = 'dashboard' | 'bloc' | 'timeline' | 'milestones' | 'actions';
export type P0SortMode = 'priorite' | 'progression' | 'budget' | 'deadline';

export function useKOSP0Execution() {
  const [activeTab, setActiveTab] = useState<P0ViewTab>('dashboard');
  const [activeBlocId, setActiveBlocId] = useState<string | null>(null);
  const [sortMode, setSortMode] = useState<P0SortMode>('priorite');
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [expandedActions, setExpandedActions] = useState<Set<string>>(new Set());
  const [expandedBlocs, setExpandedBlocs] = useState<Set<string>>(new Set(P0_EXECUTION_BLOCKS.map(b => b.id)));
  ...
  const kpis = useMemo(() => computeP0ExecutionKPIs(), []);
  ...
  const sortedBlocs = useMemo(() => {
    ...
    if (sortMode === 'progression') blocs.sort((a, b) => a.progressionGlobale - b.progressionGlobale);
      ...
      const getEarliest = (b: P0ExecutionBlock) => b.actions.map(a => a.deadline).sort()[0] || '';
  ...
  const activeBloc = useMemo(
    () => (activeBlocId ? P0_EXECUTION_BLOCKS.find(b => b.id === activeBlocId) || null : null),
    [activeBlocId],
  );
  ...
  const milestones = useMemo(() => P0_GLOBAL_MILESTONES, []);
  const timeline = useMemo(() => P0_TIMELINE, []);
  const executiveSummary = useMemo(() => P0_EXECUTIVE_SUMMARY, []);
