...
import { KOS_BLOG_WRITING_AUTOMATES as mockAutomates, BLOG_WRITING_CATEGORIES as mockCategories, BLOG_WRITING_AUTOMATES_KPIS as mockKPIs } from '@/mocks/kosBlogWritingAutomates';
import type { KOSBlogWritingAutomate, BlogWritingAutomateCategory } from '@/mocks/kosBlogWritingAutomates';

export type { KOSBlogWritingAutomate, BlogWritingAutomateCategory };
...
export interface BlogWritingAutomatesData {
  automates: KOSBlogWritingAutomate[];
  categories: BlogWritingAutomateCategory[];
  kpis: BlogWritingAutomatesKPIs;
  isLive: boolean;
}

function mapAutomate(row: Record<string, unknown>): KOSBlogWritingAutomate {
  return {
    id: row.id as string,
    name: row.name as string,
    category: row.category as string,
    tech_stack: Array.isArray(row.tech_stack) ? row.tech_stack as string[] : [],
    status: row.status as KOSBlogWritingAutomate['status'],
    ...
    priority: row.priority as KOSBlogWritingAutomate['priority'],
    kpis: Array.isArray(row.kpis) ? row.kpis as KOSBlogWritingAutomate['kpis'] : [],
  };
}
