...
export interface KOSJournalEntry {
  id: number;
  actionId: string;
  date: string;
...
function computeKPIs(actions: KOSAction[], sprints: KOSSprint[]) {
...
export function useKOSPhase1P0Immediate() {
  ...
  const [actions, setActions] = useState<KOSAction[]>([]);
  const [sprints, setSprints] = useState<KOSSprint[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const mappedActions: KOSAction[] = rawActions.map((r: Record<string, unknown>) => ({
        id: r.id as string,
        ...
        statut: (r.statut as KOSAction['statut']) || 'a_faire',
      ...
      const mappedSprints: KOSSprint[] = rawSprints.map((r: Record<string, unknown>) => ({
        id: r.id as number,
        phase: r.phase as string,
        ...
        progression: (r.progression as number) || 0,
        statut: (r.statut as KOSSprint['statut']) || 'a_venir',
      }));
  ...
  const getActionById = (id: string): KOSAction | undefined => actions.find(a => a.id === id);

  const getSprintActions = (sprintNum: number): KOSAction[] => {
    const sprintActionIds = sprints.find(s => s.numero === sprintNum)?.actions || [];
    return actions.filter(a => sprintActionIds.includes(a.id));
  };

  const updateActionStatus = async (actionId: string, statut: KOSAction['statut']) => {
  ...
  const addJournalEntry = async (actionId: string, type: KOSJournalEntry['type'], message: string) => {
