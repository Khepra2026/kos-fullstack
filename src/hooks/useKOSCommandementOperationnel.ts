...
} from '@/mocks/kosCommandementOperationnelUnifie';
import type {
  KOSAutomateFamily,
  KOSUnifiedGlobalKPIs,
  KOSOperationalChain,
} from '@/mocks/kosCommandementOperationnelUnifie';

export type { KOSAutomateFamily, KOSUnifiedGlobalKPIs, KOSOperationalChain };

export interface KOSCommandementData {
  families: KOSAutomateFamily[];
  globalKPIs: KOSUnifiedGlobalKPIs;
  operationalChain: KOSOperationalChain[];
...
export function useKOSCommandementOperationnel() {
  const [data, setData] = useState<KOSCommandementData>({
    families: [],
    globalKPIs: {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const liveFamilies: KOSAutomateFamily[] = mockFamilies.map(mock => {
      ...
      const liveKPIs: KOSUnifiedGlobalKPIs = {
