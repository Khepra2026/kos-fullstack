...
export const useGlobalSearch = (query: string) => {
  ...
  useEffect(() => {
    ...
    const fetchLiveResults = async () => {
      ...
      try {
        const [ragResult, kosResult] = await Promise.allSettled([
          supabase
            .from('rag_documents')
        ...
        if (kosResult.status === 'fulfilled' && !kosResult.value.error && kosResult.value.data && kosResult.value.data.length > 0) {
          kosResult.value.data.forEach((page: Record<string, unknown>) => {
            ...
            mapped.push({
              id: `kospage-${page.slug as string}`,
              type: mappedType,
