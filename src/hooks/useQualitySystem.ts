...
export function useQualitySystem(): UseQualitySystemReturn {
  const [globalReport, setGlobalReport] = useState<KOSGlobalReport>(KOS_GLOBAL_REPORT);
  const [agents, setAgents] = useState<KOSAgent[]>(KOS_AGENTS);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (hasSupabaseData) {
        ...
        const mappedAgents: KOSAgent[] = agentsRes.data.map((a: any) => ({
          id: a.id,
          number: a.number,
          name: a.name,
          mission: a.mission,
