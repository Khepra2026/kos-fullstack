...
import { KOS_WEB_OPS_AUTOMATES as mockAutomates, WEB_OPS_AUTOMATE_CATEGORIES as mockCategories, WEB_OPS_AUTOMATES_KPIS as mockKPIs } from '@/mocks/kosWebOpsAutomates';
import type { KOSWebOpsAutomate, WebOpsAutomateCategory } from '@/mocks/kosWebOpsAutomates';

export type { KOSWebOpsAutomate, WebOpsAutomateCategory };
...
export interface WebOpsAutomatesData {
  automates: KOSWebOpsAutomate[];
  categories: WebOpsAutomateCategory[];
  kpis: WebOpsAutomatesKPIs;
  isLive: boolean;
}

function mapAutomate(row: Record<string, unknown>): KOSWebOpsAutomate {
  return {
    id: row.id as string,
    name: row.name as string,
    category: row.category as string,
    tech_stack: (row.tech_stack as string[]) || [],
    status: row.status as KOSWebOpsAutomate['status'],
    ...
    priority: row.priority as KOSWebOpsAutomate['priority'],
    kpis: (row.kpis as KOSWebOpsAutomate['kpis']) || [],
  };
}
