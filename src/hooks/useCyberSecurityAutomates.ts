...
import { KOS_CYBER_SECURITY_AUTOMATES as mockAutomates, CYBER_SECURITY_CATEGORIES as mockCategories, CYBER_SECURITY_AUTOMATES_KPIS as mockKPIs } from '@/mocks/kosCyberSecurityAutomates';
import type { KOSCyberSecurityAutomate, CyberSecurityAutomateCategory } from '@/mocks/kosCyberSecurityAutomates';

export type { KOSCyberSecurityAutomate, CyberSecurityAutomateCategory };
...
export interface CyberSecurityAutomatesData {
  automates: KOSCyberSecurityAutomate[];
  categories: CyberSecurityAutomateCategory[];
  kpis: CyberSecurityAutomatesKPIs;
  isLive: boolean;
}

function mapAutomate(row: Record<string, unknown>): KOSCyberSecurityAutomate {
  return {
    id: row.id as string,
    name: row.name as string,
    category: row.category as string,
    tech_stack: Array.isArray(row.tech_stack) ? row.tech_stack as string[] : [],
    status: row.status as KOSCyberSecurityAutomate['status'],
    ...
    priority: row.priority as KOSCyberSecurityAutomate['priority'],
    kpis: Array.isArray(row.kpis) ? row.kpis as KOSCyberSecurityAutomate['kpis'] : [],
  };
}
