...
export interface KOSTrainingLog {
  id: number;
  agent_id: string;
  question: string;
  answer: string;
...
export function useKOSAgents() {
  const [agents, setAgents] = useState<KOSAgent[]>([]);
  const [logs, setLogs] = useState<KOSTrainingLog[]>([]);
  ...
  const fetchAgents = useCallback(async () => {
    try {
      ...
      const typedAgents: KOSAgent[] = (agentsData || []).map((a) => ({
        ...a,
        metadata: (a.metadata as Record<string, unknown>) || {},
      }));

      const typedLogs: KOSTrainingLog[] = (logsData || []).map((l) => ({
        ...l,
        sources: l.sources || [],
