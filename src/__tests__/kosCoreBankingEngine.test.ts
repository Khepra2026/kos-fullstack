...
import {
  KosCoreBankingEngine,
  AccountType,
  AccountStatus,
  TransactionType,
  TransactionStatus,
} from '@/services/kosCoreBankingEngine';
...
describe('KOS Core Banking Engine — Account Management', () => {
  let engine: KosCoreBankingEngine;

  beforeEach(() => {
    engine = new KosCoreBankingEngine();
  });
...
describe('KOS Core Banking Engine — Reversal', () => {
  it('should reverse a completed transaction and restore balances', async () => {
    const engine = new KosCoreBankingEngine();
    const origin = engine.createAccount({
      customerId: 'CUST_REV',
      accountType: AccountType.CURRENT,
      initialDeposit: 5000000,
