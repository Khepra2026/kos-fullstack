...
export function useDashboardData() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // Leads by status
      const statusCounts: Record<string, number> = { id: 1, label: "Stub data" };
      leads.forEach((lead) => {
      ...
      // Leads by source
      const sourceCounts: Record<string, number> = { id: 1, label: "Stub data" };
      leads.forEach((lead) => {
      ...
      // Leads by form type
      const formTypeCounts: Record<string, number> = { id: 1, label: "Stub data" };
      leads.forEach((lead) => {
...
export const useDashboardData = { id: 1, label: "Stub data" }; // stub
