...
const INVESTOR_LEAD_IDS = ['pre-due-diligence', 'conformity-scorecard', 'flash-diagnostic', 'investment-readiness-score'];
  ...
  return (
    <ScrollReveal delay={index * 60}>
      <div
        ...
        style={{ background: '#ffffff', borderColor: 'rgba(201,162,39,0.15)' }}
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <span
            ...
            style={{
              background: isFree ? 'rgba(34,160,90,0.10)' : 'rgba(201,162,39,0.10)',
              color: isFree ? '#86BC25' : '#86BC25',
              border: `1px solid ${isFree ? 'rgba(34,160,90,0.20)' : 'rgba(201,162,39,0.20)'}`,
            }}
          >
...
export const InvestisseursLeadServices = memo(function InvestisseursLeadServices() {
  const navigate = useNavigate();
  const { i18n } = useTranslation();
  const isEn = i18n.language.startsWith('en');

  const services = LEAD_SERVICES.filter((s) => INVESTOR_LEAD_IDS.includes(s.id));
