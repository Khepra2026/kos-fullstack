...
} from '@/mocks/finalOrchestration';
import type { hubRegistryEntry, unifiedGlobalState, crossSystemAlert, systemLink, resourceAllocation } from '@/mocks/finalOrchestration';

export function useKOSFinalOrchestration() {
  const [hubs, setHubs] = useState<hubRegistryEntry[]>([]);
  const [globalState, setGlobalState] = useState<unifiedGlobalState | null>(null);
  const [crossAlerts, setCrossAlerts] = useState<crossSystemAlert[]>([]);
