...
export interface masterSyncData {
  families: familySync[];
  globalKPIs: masterGlobalKPIs;
  kpoDimensions: kPODimension[];
...
export function useKOSMasterSynchronizer() {
  const [data, setData] = useState<masterSyncData>({
    families: [],
    globalKPIs: mockKPIs,
    kpoDimensions: [],
    gapAnalysis: [],
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const liveFamilies: familySync[] = mockFamilies.map(mock => {
        ...
        let syncStatus: familySync['sync_status'] = 'synced';
      ...
      const liveKPIs: masterGlobalKPIs = {
        ...mockKPIs,
