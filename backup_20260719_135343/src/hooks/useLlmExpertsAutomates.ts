...
import { KOS_LLM_EXPERTS_AUTOMATES as mockAutomates, LLM_EXPERTS_CATEGORIES as mockCategories, LLM_EXPERTS_KPIS as mockKPIs } from '@/mocks/llmExpertsAutomates';
import type { llmExpertsAutomate, LlmExpertsCategory } from '@/mocks/llmExpertsAutomates';

export type { llmExpertsAutomate, LlmExpertsCategory };
...
export interface LlmExpertsData {
  automates: llmExpertsAutomate[];
  categories: LlmExpertsCategory[];
  kpis: LlmExpertsKPIs;
  isLive: boolean;
}

function mapAutomate(row: Record<string, unknown>): llmExpertsAutomate {
  return {
    id: row.id as string,
    ...
    status: row.status as llmExpertsAutomate['status'],
    ...
    priority: row.priority as llmExpertsAutomate['priority'],
    kpis: Array.isArray(row.kpis) ? row.kpis as llmExpertsAutomate['kpis'] : [],
