...
export interface globalLaunchData {
  blocks: globalBlock[];
  globalStats: globalLaunchStats;
  logs: globalLaunchLog[];
  isLive: boolean;
}

export function useKOSGlobalLaunch() {
  const [data, setData] = useState<globalLaunchData>({
    blocks: mockBlocks,
  ...
  const addLog = useCallback((log: globalLaunchLog) => {
    setData(prev => ({
      ...prev,
      logs: [log, ...prev.logs],
    }));
  ...
  const launchBlock = useCallback(async (blockId: string) => {
    ...
    for (const task of sortedTasks) {
      ...
      const inProgressLog: globalLaunchLog = {
      ...
      const completedLog: globalLaunchLog = {
  ...
  const launchAllBlocks = useCallback(async () => {
    ...
    for (const blockId of blockOrder) {
      ...
      const blockLaunchLog: globalLaunchLog = {
      ...
      const blockCompleteLog: globalLaunchLog = {
    ...
    const finalLog: globalLaunchLog = {
  ...
  const launchByPriority = useCallback(async (priority: 'critical' | 'urgent' | 'planned') => {
    ...
    const allPriorityTasks: { blockId: string; task: globalTask }[] = [];
    ...
    const finalPriorityLog: globalLaunchLog = {
