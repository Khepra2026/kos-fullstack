...
function mapEngine(row: Record<string, unknown>): engineResource {
  return {
    id: row.id as string,
...
function mapAgent(row: Record<string, unknown>): unifiedAgent {
  return {
    id: row.id as string,
    name: row.name as string,
    ...
    kpis: (row.kpis as unifiedAgent['kpis']) || [],
    deploymentVersion: row.deployment_version as string,
    charter: row.charter as string | null,
    autoDeploy: Boolean(row.auto_deploy),
    alerts: (row.alerts as unifiedAgent['alerts']) || [],
  };
}

function mapHealth(row: Record<string, unknown>): systemHealth {
  return {
