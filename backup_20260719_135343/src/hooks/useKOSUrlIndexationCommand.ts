...
import {
  urlIndexationOverview,
  urlIndexationCategories,
  urlIndexationCauses,
  urlIndexationExcludedList,
  urlIndexationActionPlan,
  urlIndexationProgress,
  urlIndexationSitemaps,
  urlIndexationTopIndexed,
  urlIndexationGSCQueries,
} from "@/mocks/urlIndexationCommand";
...
export interface UseKOSUrlIndexationCommandReturn {
  ...
  categories: typeof urlIndexationCategories;
  causes: typeof urlIndexationCauses;
...
export function useKOSUrlIndexationCommand(): UseKOSUrlIndexationCommandReturn {
  const [overview, setOverview] = useState<IndexationOverview>(urlIndexationOverview);
  const [excludedUrls, setExcludedUrls] = useState<UrlIndexationEntry[]>(urlIndexationExcludedList);
  const [actionPlan, setActionPlan] = useState<ActionPlanEntry[]>(urlIndexationActionPlan);
  const [sitemaps, setSitemaps] = useState<SitemapStatus[]>(urlIndexationSitemaps);
  const [topIndexed, setTopIndexed] = useState<TopIndexedPage[]>(urlIndexationTopIndexed);
  const [gscQueries, setGscQueries] = useState<GSCQuery[]>(urlIndexationGSCQueries);
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      setOverview(urlIndexationOverview);
      setExcludedUrls(urlIndexationExcludedList);
      setActionPlan(urlIndexationActionPlan);
      setSitemaps(urlIndexationSitemaps);
      setTopIndexed(urlIndexationTopIndexed);
      setGscQueries(urlIndexationGSCQueries);
      setDataSource("mock");
    } catch (err: any) {
  ...
  const executeAction = useCallback(async (actionId: string) => {
    ...
    const action = urlIndexationActionPlan.find(a => a.id === actionId);
    if (!action) {
    ...
    const urlsForAction = urlIndexationExcludedList.filter(u => {
  ...
  const executeAllActions = useCallback(async () => {
    ...
    const pendingActions = urlIndexationActionPlan.filter(a => a.status !== "completed");
  ...
  return {
    overview,
    categories: urlIndexationCategories,
    causes: urlIndexationCauses,
    excludedUrls,
    actionPlan,
    progress: urlIndexationProgress,
    sitemaps,
    topIndexed,
    gscQueries,
    loading,
