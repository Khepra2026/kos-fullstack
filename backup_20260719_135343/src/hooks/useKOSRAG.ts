...
export interface rAGResult {
  ...
  kos_enrichment: {
    ...
    voisins_enrichis: enrichedNeighbor[];
...
export interface enrichedNeighbor {
  nodeId: string;
  label: string;
  type: string;
  hops: number;
...
export function useKOSRAG(): UseKOSRAGReturn {
  ...
  const graphRef = useRef<graph>(getGlobalKOSGraph());
  ...
  const query = useCallback(async (
    ...
    try {
      ...
      const voisinsEnrichis: enrichedNeighbor[] = [];
      ...
      if (enrichKOS) {
        ...
        if (filterTheme) {
          ...
          for (const themeNode of allThemeNodes.slice(0, 3)) {
            const related = graph.findRelated(themeNode.id, hops);
