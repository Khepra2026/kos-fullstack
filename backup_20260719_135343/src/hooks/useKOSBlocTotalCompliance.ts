...
import { BLOC_SCAN_MODULES } from '@/mocks/blocTotalCompliance';
import type { BlocScanModule, BlocTotalSummary, Finding } from '@/mocks/blocTotalCompliance';
...
export interface routineExecutionResult {
  ...
  report: routineBigFourReport | null;
  error: string | null;
  iso_clause: string;
}

export interface routineBigFourReport {
  format: string;
...
export interface routineEngineState {
  routines: routine[];
  routineMetrics: routineMetrics[];
  executingRoutineId: string | null;
  lastExecutionResult: routineExecutionResult | null;
...
export function useKOSBlocTotalCompliance(showToast?: ToastFn) {
  ...
  const [automaton, setAutomaton] = useState<automatonState>({
    phase: 'idle',
    coverageGaps: [],
  ...
  const [routineEngine, setRoutineEngine] = useState<routineEngineState>({
    routines: [],
    routineMetrics: [],
  ...
  const loadRoutines = useCallback(async () => {
    try {
      ...
      const routines: routine[] = (data.new_state.routines || []).map((r: any) => ({
        id: r.id || '',
        name: r.name || '',
  ...
  const executeRoutine = useCallback(async (routineId: string): Promise<routineExecutionResult | null> => {
    ...
    try {
      ...
      const errorResult: routineExecutionResult = {
        success: false,
        execution_id: '',
