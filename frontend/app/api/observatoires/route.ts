...
export async function GET(){
  ...
  try {
    if(url && key){
      const r = await fetch(url+'/rest/v1/kos_observatoires?select=*&limit=10', {
  ...
  return NextResponse.json({
    ...
    data: data || { message: 'Set SUPABASE env in Vercel to get real data', table: 'kos_observatoires' },
    evidence_id,
