  ...
  try {
    ...
    if (!rag.data || rag.data.length === 0) {
      return new Response(JSON.stringify({
        ...
        error: "Aucune source OHADA trouvée. Refus de répondre pour éviter hallucination.",
    ...
    const response = {
      ...
      focus: 'Achats, appels d'offres, fournisseurs',
      data_residency: "CEMAC",
      audit_trail: true,
    ...
    console.error('[Procurement_Agent] Error:', e.message);
    return new Response(JSON.stringify({
      agent: 'Procurement_Agent',
      error: e.message
    }), {
